...
class OriginOneTimeContractsSetup {
  ...
  perform() {
    ...
    return oThis._asyncPerform().catch(function(error) {
      if (responseHelper.isCustomResult(error)) {
        ...
        logger.error('tools/setup/OriginOneTimeContracts.js::perform::catch');
        logger.error(error);
  ...
  async _asyncPerform() {
    ...
    await oThis._setupOriginOrganization(chainAddressConstants.baseContractOrganizationKind);

    await basicHelper.pauseForMilliSeconds(200);

    logger.step('* Deploying organization for anchor.');
    await oThis._setupOriginOrganization(chainAddressConstants.anchorOrganizationKind);
