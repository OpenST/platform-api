...
class GetTransactionDetails {
  ...
  constructor(params) {
    ...
    oThis.txUuids = params.txUuids;

    logger.log('oThis.txUuids ====', oThis.txUuids);

    oThis.blockScanner = null;
    oThis.blockHeight = null;

    oThis.txUuidToTxHashMap = {};
  ...
  async _checkTxHashStatus() {
    ...
    if (oThis.esSearchResponse && oThis.esSearchResponse.isSuccess()) {
      const transactionsFromEs = oThis.esSearchResponse.data[oThis.chainId + '_transactions']; // Get this from core constants.
      ...
      for (let index = 0; index < transactionsFromEs.length; index++) {
        ...
        oThis.txUuidToTxHashMap[currentTransaction.id] = currentTransaction.transaction_hash;
      }
    }
  }
  ...
  async _fetchTxFromPendingTxTable() {
    ...
    if (oThis.sortedTransactionUuids.length === 0 && oThis.txUuids.length) {
      logger.log('Inside fallback es code.');
      oThis.uuidsToFetchFromPendingTx = oThis.uuidsToFetchFromPendingTx.concat(oThis.txUuids);
      oThis.sortedTransactionUuids = oThis.sortedTransactionUuids.concat(oThis.txUuids);

      // Remove duplicates.
      oThis.uuidsToFetchFromPendingTx = [...new Set(oThis.uuidsToFetchFromPendingTx)];
      oThis.sortedTransactionUuids = [...new Set(oThis.sortedTransactionUuids)];

      logger.log('oThis.uuidsToFetchFromPendingTx ==========', oThis.uuidsToFetchFromPendingTx);
      logger.log('oThis.sortedTransactionUuids ==========', oThis.sortedTransactionUuids);
    }
    ...
    for (let index = 0; index < oThis.uuidsToFetchFromPendingTx.length; index++) {
      ...
      if (basicHelper.isEmptyObject(txDetails)) {
        oThis.missingTxUuids.push(currentTxUuid);

        let txHash = oThis.txUuidToTxHashMap[currentTxUuid];
        if (txHash) {
          oThis.txHashes.push(txHash);
        }

        continue;
      }

      let blockNo = txDetails.blockNumber || 0,
    ...
    if (oThis.missingTxUuids.length > 0) {
      logger.debug(
        'Some UUIDS were to be found in Pending Tx Table but were not. oThis.missingTxUuids:',
        oThis.missingTxUuids
      );
    }
  }
  ...
  async _fetchTxFromTxTable() {
    ...
    for (let txHash in txMap) {
      ...
      if (Number(txDetails.transactionStatus) & Number(txDetails.transactionInternalStatus)) {
        ...
        txDetails.status = pendingTransactionConstants.successStatus;
      } else {
        txDetails.transfers = oThis._setTransfers(txDetails.transfers);
        txDetails.status = pendingTransactionConstants.failedStatus;
      }
