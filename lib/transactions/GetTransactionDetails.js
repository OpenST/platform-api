...
class GetTransactionDetails {
  ...
  async _checkTxHashStatus() {
    const oThis = this,
      invertedSuccessStatus = +pendingTransactionConstants.invertedStatuses[pendingTransactionConstants.successStatus],
      invertedFailedStatus = +pendingTransactionConstants.invertedStatuses[pendingTransactionConstants.failedStatus],
    ...
    for (let index = 0; index < transactionsFromEs.length; index++) {
      ...
      if (
        currentTransaction.status === invertedSuccessStatus ||
        (currentTransaction.status === invertedFailedStatus &&
          !CommonValidators.isVarNull(currentTransaction.transaction_hash))
      ) {
        oThis.transactionHashes.push(currentTransaction.transaction_hash);
      } else {
  ...
  async _getPendingTx() {
    ...
    for (let index = 0; index < oThis.pendingTransactionUuids.length; index++) {
      ...
      if (!pendingTxHashesResponseData[currentTxUuid]) {
        ...
        oThis._setRuleIds(transactionHashDetails);
        oThis.transactionDetailsMap[currentTxUuid] = transactionHashDetails;
      }
    }
    logger.debug(' ===== oThis.transactionDetailsMap', oThis.transactionDetailsMap);
  }
  ...
  async _fetchTxDetailsFromBlockScanner() {
    ...
    let transfersResponse = await oThis._fetchTransfers(),
      transactionTransfers = transfersResponse.data;
    ...
    for (let transactionHash in transactionDetailsData) {
      ...
      if (Number(transactionHashDetails.transactionStatus) & Number(transactionHashDetails.transactionInternalStatus)) {
        if (CommonValidators.validateObject(transactionTransfers[transactionHash])) {
          let transferResponseData = transactionTransfers[transactionHash];
          transactionHashDetails['transfers'] = oThis._setTransfers(transferResponseData);
        }
      ...
      oThis.transactionDetailsMap[transactionUuid] = transactionHashDetails;
    }
    logger.debug(' ===== oThis.transactionDetailsMap', oThis.transactionDetailsMap);
  }
  ...
  _fetchTransfers() {
    const oThis = this,
      ...
      getTransferDetailObj = new GetTransferDetail(oThis.chainId, oThis.transactionHashes);
  ...
  _setTransfers(transferDetails) {
    ...
    for (let transferIndex in transferDetails) {
      let transferDetails = transferDetails[transferIndex];
  ...
  /**
   * Store rule ids to fetch rule names.
   *
   * @param transactionHashDetails
   *
   * @private
   */
  _setRuleIds(transactionHashDetails) {
    const oThis = this;
    ...
    if (transactionHashDetails.ruleId) {
      ...
      oThis.ruleIdToTxUuidMap[transactionHashDetails.ruleId].push(transactionHashDetails.transactionUuid);
  ...
  async _setUserIdsAndRuleNames() {
    ...
    for (let index = 0; index < oThis.sortedTransactionUuids.length; index++) {
      ...
      oThis.transactions.push(txDetailsForUuid);
    }
  }
