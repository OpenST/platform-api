...
class GetTransactionDetails {
  ...
  async perform() {
    ...
    if (oThis.txTransactionHashes.length) {
  ...
  async _checkTxHashStatus() {
    ...
    for (let index = 0; index < transactionsFromEs.length; index++) {
      ...
      if (currentTransaction.status === invertedSuccessStatus) {
        if (!CommonValidators.isVarNull(currentTransaction.transaction_hash)) {
  ...
  async _getPendingTx() {
    ...
    console.log(' ===== oThis.finalTransactionDetails', oThis.finalTransactionDetails);
  }
  ...
  _fetchTransfers() {
    const oThis = this,
      ...
      getTransferDetailObj = new GetTransferDetail(oThis.chainId, oThis.txTransactionHashes);
