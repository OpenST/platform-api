...
const rootPrefix = '../..',
  ...
  createPendingTransactionKlass = require(rootPrefix + '/lib/transactions/createPendingTransaction'),
...
class SignSubmitTrxOnChain {
    ...
    return new Promise(async function(onResolve, onReject) {
      const afterSubmitCallback = function(err, txHash) {
        ...
        oThis._createPendingTransaction(txHash).then(function(resp) {
          return onResolve(responseHelper.successWithData({ transactionHash: txHash }));
        });
      };
  ...
  _createPendingTransaction(txHash) {
    ...
    return new createPendingTransactionKlass(oThis.chainId).insertPendingTransaction(oThis.pendingTransaction, txHash);
  }
