  ...
  async perform() {
    ...
    return new Promise(async function(onResolve, onReject) {
      const afterSubmitCallback = async function(err, txHash) {
        ...
        if (oThis.extraData) {
          let toReject = false;
          await oThis._createPendingTransaction(txHash).catch(function() {
            toReject = true;
          });
        ...
        if (oThis.waitTillReceipt) {
          await oThis._getTxReceipt(txHash);
        }
      ...
      Object.assign(oThis.pendingTransaction, { rawTransaction: signedTransaction });
