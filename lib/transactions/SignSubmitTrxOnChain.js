...
class SignSubmitTrxOnChain {
   ...
   * @param params.pendingTransactionKind {String} - pending transaction kind
   * @param [params.web3Instance] {Object} - Web 3 Instance
   * @param [params.provider] {String} - Web 3 provider to create web 3 instance
   * @param [params.providers] {array} - Web 3 provider to create web 3 instance
  ...
  constructor(params) {
    ...
    oThis.web3Providers = params.providers;
  ...
  async _afterSubmitCallback(err, txHash) {
    ...
    if (oThis.waitTillReceipt) {
      let transactionReceipt = await oThis._getTxReceipt(txHash).catch(function(err) {
        let errorObject = null;
        if (responseHelper.isCustomResult(err)) {
          errorObject = err;
        } else {
          errorObject = responseHelper.error({
            internal_error_identifier: 'l_t_sstoc_11',
            api_error_identifier: 'something_went_wrong',
            debug_options: { errMsg: 'Error obtained while fetching tx receipt in sign submit trx on chain.' }
          });
        }

        logger.error('Error in Sign Submit Trx on Chain ', errorObject);
        createErrorLogsEntry.perform(errorObject, ErrorLogsConstants.highSeverity);

        // Passing the reject to the promise context. And handling it here.
        oThis.promiseContext.reject(errorObject);

        return false;
      });

      // If reject obtained in tx receipt get, return from here. No need to resolve.
  ...
  async _getSiblingGethProvider() {
    ...
    if (!oThis.siblingProviders) {
      ...
      if (chainConfig.auxGeth) {
        ...
        } else if (chainConfig.auxGeth.readWrite.rpcProviders.includes(oThis.web3ProviderPath)) {
          oThis.siblingProviders = chainConfig.auxGeth.readWrite.rpcProviders;
        }
      ...
      } else if (chainConfig.originGeth.readWrite.rpcProviders.includes(oThis.web3ProviderPath)) {
        oThis.siblingProviders = chainConfig.originGeth.readWrite.rpcProviders;
      }
  ...
  _getTxReceipt(transactionHash) {
    ...
    // wait for receipt for approx 30 mins.
        ...
        if (!maxRetries)
          return onReject(
            responseHelper.error({
              ...
              api_error_identifier: 'something_went_wrong',
              debug_options: {
                errMsg: 'max retries for getting tx receipt exceeded for tx hash: ' + transactionHash,
  ...
  async _fetchPrivateKey() {
    ...
    let privateKeyObj = new Buffer.from(privateKey, 'hex');
  ...
  async _initializeWeb3Instance() {
    ...
    if (oThis.web3Instance) {
      oThis._getHost();
    } else if (oThis.web3Providers) {
      oThis.siblingProviders = basicHelper.shuffleArray(oThis.web3Providers);
      oThis.web3ProviderPath = oThis.siblingProviders[0];
    }

    if (!oThis.web3ProviderPath) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_t_sstoc_10',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
            errMsg: 'web3Instance or provider is mandatory..'
          }
        })
      );
    }
  ...
  async _fetchNonce() {
    ...
    if (!CommonValidators.isVarNull(oThis.txOptions.nonce)) {
      return;
    }
