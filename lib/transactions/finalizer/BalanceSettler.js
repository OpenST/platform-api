...
class BalanceSettler {
  ...
  async _prepareLatestTransactionData() {
    ...
    //gas price , gas limit
    oThis.latestTransactionsArray = [];

    for (let transactionHash in oThis.transactionTransfersMap) {
      if (!oThis.txHashToTxMetaDataMap[transactionHash].tokenId) {
        //This is the case when tx is not found in tx meta table. Thus it is not a user transaction.
        continue;
      }
      for (let transferNumber in oThis.transactionTransfersMap[transactionHash]) {
        let transactionDataMap = {};
        transactionDataMap.transactionHash = transactionHash;
        transactionDataMap.gasPrice = oThis.txHashToTransactionDataMap[transactionHash].gasPrice;
        transactionDataMap.gasUsed = oThis.txHashToTransactionDataMap[transactionHash].gasUsed;
        transactionDataMap.tokenId = oThis.txHashToTxMetaDataMap[transactionHash].tokenId;
        transactionDataMap.blockTimestamp = oThis.txHashToTransactionDataMap[transactionHash].blockTimestamp;
        transactionDataMap.amount = oThis.transactionTransfersMap[transactionHash][transferNumber].amount;
        transactionDataMap.chainId = oThis.auxChainId;
        oThis.latestTransactionsArray.push(transactionDataMap);
      }
    }

    console.log('---oThis.latestTransactionsArray---', oThis.latestTransactionsArray);
  }

  async _publishLatestTransactions() {
    const oThis = this;

    if (Object.keys(oThis.latestTransactionsArray).length === 0) {
      return;
    }
    ...
    const messageParams = {
      ...
      message: {
        ...
        payload: JSON.stringify(oThis.latestTransactionsArray)
      }
    };
  ...
  async _postTxFinalizationSteps() {
    ...
    const postTxFinalizeSteps = new PostTxFinalizeSteps({
      ...
      transactionTransfersMap: oThis.transactionTransfersMap
    });
