...
class BalanceSettler {
  ...
  async _publishWebhookPreprocessorMessages() {
    ...
    for (let index = 0; index < oThis.webhookPreprocessorPayloadsArray.length; index++) {
      preprocessorPublishPromisesArray.push(
        preProcessorPublish.perform(oThis.auxChainId, oThis.webhookPreprocessorPayloadsArray[index])
      );
    }
  ...
  async _releaseLock() {
    ...
    const successIds = [];
    for (let index = 0; index < oThis.lockedMetaIds.length; index++) {
      const id = oThis.lockedMetaIds[index];
      if (!failedIdsMap[id]) {
        successIds.push(id);
      }
    }
