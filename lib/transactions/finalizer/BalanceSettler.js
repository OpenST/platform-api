...
class BalanceSettler {
  ...
  async _updateStatusesInDb() {
    ...
    for (let i = 0; i < oThis.transactionHashes.length; i++) {
      ...
      if (txMetaData) {
        ...
        let transfersInfo = oThis.transactionTransfersMap[txHash];
        if (CommonValidators.validateObject(transfersInfo)) {
      ...
      if (CommonValidators.validateObject(pendingTxData)) {
        ...
        if (
          ...
          if (txMissingInPendingTx) {
            updateQueryParams['transactionHash'] = txHash;
          }
  ...
  async _fetchLockAcquiredTransactions() {
    ...
    //TODO: Should we also handle failure of this select by releasing lock ?
  ...
  async _getTransferDetails() {
    const oThis = this,
      ...
      fetchTransfers = new Transfer(oThis.auxChainId, oThis.transactionHashes),
      transferResp = await fetchTransfers.perform();

    if (transferResp.isFailure()) {
      return Promise.reject(transferResp);
    }

    oThis.transactionTransfersMap = transferResp.data;
