...
const rootPrefix = '../..',
  ...
  AcceptUserRedemptionRequests = require(rootPrefix + '/lib/redemption/userRequests/Accept'),
...
class ProcessPendingTransaction {
  ...
  async perform() {
    ...
    await oThis._acceptRedemptionRequests();
   ...
   * @private
   */
  async _acceptRedemptionRequests() {
    const oThis = this;
    ...
    for (let txHash in oThis.txHashPendingTxDataMap) {
      ...
      if (ptd.hasOwnProperty('redemptionDetails') && ptd.redemptionDetails.redemption_id) {
        userRedemptionIds.push(ptd.redemptionDetails.redemption_id);
      }
    }

    // If there are any redemption requests in finalized transactions
    if (userRedemptionIds.length > 0) {
      await new AcceptUserRedemptionRequests({
