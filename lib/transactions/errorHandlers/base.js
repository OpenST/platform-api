...
class ErrorHandlerBaseKlass {
  ...
  async _rollBackPessimisticDebit() {
    ...
    for (let i = 0; i < oThis.failMarkedTransactionUuids.length; i++) {
      ...
      let balanceShardNumber = await oThis._setBalanceShardNumber(txMeta.token_id);
      ...
      let unsettledDebitsArray = pendingTransaction.unsettledDebits;

      for (let i = 0; i < unsettledDebitsArray.length; i++) {
        let unsettledDebits = unsettledDebitsArray[i];
        let blockChainUnsettleDebitsBn = new BigNumber(unsettledDebits['blockChainUnsettleDebits']);

        unsettledDebits['blockChainUnsettleDebits'] = blockChainUnsettleDebitsBn.mul(-1).toString(10);

        let updateResponse = await balanceObj.updateBalance(unsettledDebits);
