...
class ErrorHandlerBaseKlass {
  ...
  async _markPendingTransactionFailAndSendPreprocessorWebhook() {
    ...
    for (let index = 0; index < oThis.transactionUuidsToBeFailed.length; index++) {
      ...
      const webhookProcessorPromise = publishToPreProcessor.perform(oThis.auxChainId, webhookPreprocessorPayload);
  ...
  _releaseLockAndMarkTransactionMetaFail() {
    ...
    for(let i=0;i<oThis.failMarkedTransactionUuids.length;i++){
      const txUuid = oThis.failMarkedTransactionUuids[i];
      failedMetaIds.push(oThis.txUuidToTxMetaMap[txUuid].id);
    }
    ...
    return new TransactionMetaModel()
      ...
      .where(['lock_id=? AND id in (?)', oThis.lockId, failedMetaIds])
      .fire();
  }
