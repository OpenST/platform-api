...
class CommonValidator {
  ...
  /**
   * Is wei value valid? (zero included)
   *
   * @param {String/Number} variable
   *
   * @return {*}
  ...
  static validateWeiAmountArray(array) {
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        if (!CommonValidator.validateWeiValue(array[i])) {
          return false;
        }
      }
      return true;
  ...
  static validateMetaProperty(object) {
    ...
    if (object['type'] && !whitelistedTypeMap[object['type']]) {
      return false;
    }

    if (!CommonValidator.validateAlphaSpaceString(object['name'])) {
      return false;
    }

    if (!CommonValidator.validateAlphaNumericString(object['details'])) {
      return false;
    }
