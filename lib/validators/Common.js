...
class CommonValidator {
   ...
   * @return {Boolean}
   */
  static validateBigNumberArray(array) {
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        if (!CommonValidator.validateBigNumber(array[i])) {
          return false;
        }
      }
      return true;
  ...
  static validatePersonalSignApiKey(variable, separator) {
    ...
    if (!oThis.isVarNull(variable) && typeof variable === 'string') {
      ...
      if (!api_key_addresses[0] || !oThis.validateEthAddress(api_key_addresses[0])) {
        return false;
      }

      // Invalid api signer address
      if (!api_key_addresses[1] || !oThis.validateEthAddress(api_key_addresses[1])) {
        return false;
      }
    }
  ...
  static validateDdbNextPagePayload(variable) {
    ...
    return typeof variable['lastEvaluatedKey'] === 'object';
  }
  ...
  static validateDdbPaginationIdentifier(key) {
    try {
      const paginationParams = basicHelper.decryptNextPagePayload(key);
  ...
  /**
   *
   * Is valid executable data object sent by FE
   *
   * @param variable
   * @return {boolean}
   */
  static validateExecutableData(variable) {
    if (typeof variable !== 'object') {
      return false;
    }

    if (!executableDataConstants.validDataDefinations.includes(variable.data_defination)) {
      return false;
    }

    if (executableDataConstants.version != variable.data_definition_version) {
      return false;
    }
    ...
    if (executableDataConstants.zeroValue != variable.value) {
      return false;
    }

    if (!CommonValidator.validateObject(variable.raw_calldata)) {
      return false;
    }
