...
class CommonValidator {
   ...
   * @return {boolean}
   */
  static validateNonZeroNumber(variable) {
    const oThis = this;

    if (oThis.validateNumber(variable)) {
      return Number(variable) > 0;
    }
  ...
  static validateApiRequestPath(variable) {
    ...
    const regexExpressionStr = `\/${environmentInfo.urlPrefix}\/v[0-9]{1}\/.*`,
      regexExpressionObj = new RegExp(regexExpressionStr);
  ...
  static validateRawCallData(object) {
    ...
    const whitelistedKeysMap = { method: 1, parameters: 1 };
