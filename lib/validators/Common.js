...
class CommonValidator {
  ...
  static validateWeiValue(variable) {
    try {
      let variableInBn = new BigNumber(variable);
      if (variableInBn.gte(0) && variableInBn.isInteger()) {
        return true;
      }
    } catch (e) {}
  ...
  static validateWeiAmountArray(array) {
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        ...
        if (typeof variable !== 'string' || !CommonValidator.validateWeiValue(variable)) {
          return false;
        }
      }
      return true;
