 ...
 * @constructor
 */
const FlushFromAllMemcacheInstances = function() {
  const oThis = this;
};

FlushFromAllMemcacheInstances.prototype = {
  /**
  ...
  clearCache: async function(key) {
    const oThis = this,
      endPoints = coreConstants.SHARED_MEMCACHE_SERVERS;

    logger.debug(`flushFromAllMemcacheInstances.clearCache called for: ${key}`);
    ...
    try {
      ...
      if (oThis.validateCacheKey(key) === false) {
        return Promise.reject(
          responseHelper.error({
            internal_error_identifier: 'l_ffamci_clearCache_1',
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_ffamci_clearCache_2',
      ...
      try {
        ...
        let callback = function(err, data) {
          if (err) {
            onResolve(
              responseHelper.error({
                internal_error_identifier: 'l_ffamci_clear_2',
        ...
        return onResolve(
          responseHelper.error({
            internal_error_identifier: 'l_ffamci_clear_3',
            api_error_identifier: 'cache_issue',
            debug_options: { err: err },
            error_config: errorConfig
          })
...
module.exports = new FlushFromAllMemcacheInstances();
