...
class CalculateTokenAuxFunderSTPRequirement {
   ...
   */
  perform() {
    let maxBalanceToFund = basicHelper.convertToLowerUnit(String(0), coreConstants.OSTPRIME_DECIMALS),
      thresholdBalance = basicHelper.convertToLowerUnit(String(0), coreConstants.OSTPRIME_DECIMALS),
    ...
    for (let addressKind in tokenAuxFunderConfig) {
      ...
      maxBalanceToFund = maxBalanceToFund.plus(
        basicHelper
          .convertToLowerUnit(String(tokenAuxFunderConfig[addressKind].fundAmount), coreConstants.OSTPRIME_DECIMALS)
