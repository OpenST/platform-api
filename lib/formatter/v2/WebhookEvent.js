...
class WebhookEventFormatter {
  ...
  async _addEntity() {
    ...
    switch (oThis.entityResultType) {
      ...
      case resultType.userRedemption: {
        const rawEntity = oThis.pendingWebhook.extraData.rawEntity;
        rawEntity.emailAddress = localCipher.decrypt(coreConstants.CACHE_SHA_KEY, rawEntity.emailAddress);
        formattedEntity = await new UserRedemptionFormatter(rawEntity).perform();
        break;
      }
      default: {
