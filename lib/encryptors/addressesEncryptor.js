...
const crypto = require('crypto'),
  ...
  ManagedAddressSaltCacheKlass = require(rootPrefix + '/lib/sharedCacheManagement/EncryptionSalt');
...
class AddressEncryptorKlass {
  ...
  constructor(params) {
    ...
    oThis.encryptionSaltId = params['encryptionSaltId'];
  }
  ...
  async encrypt(dataToEncrypt) {
    ...
    let response = await oThis._fetchAddressSalt(oThis.encryptionSaltId);
   ...
   *
   */
  async _fetchAddressSalt() {
    const oThis = this;

    let obj = new ManagedAddressSaltCacheKlass({ id: oThis.encryptionSaltId });
