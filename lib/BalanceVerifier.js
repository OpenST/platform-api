...
class BalanceVerifier {
  ...
  constructor(params) {
    ...
    oThis.esConfig = {};
    oThis.esSearchData = [];
    oThis.esSearchNonFinalizedData = [];
  ...
  async _getUserAddresses() {
    ...
    logger.log('Finalized user addresses: ', oThis.finalizedUserAddresses);
    logger.log('Not finalized user addresses: ', oThis.notFinalizedUserAddresses);
    logger.log('Token id to user addresses map: ', oThis.tokenIdToUserAddressesMap);
  }
  ...
  async _verifyBalance() {
    ...
    for (const address of oThis.finalizedUserAddresses) {
      ...
      if (!oThis.notFinalizedUserAddresses.has(address)) {
        const ddbBalance = oThis.userAddrToDdbBalancesMap[address.toLowerCase()] || 0,
          gethBalance = oThis.userAddrToGethBalanceMap[address.toLowerCase()] || 0,
        ...
        if (!zeroBigNumber.equals(blockChainUnsettleDebitsBn)) {
          ...
          logger.log('====USD non zero for', address, blockChainUnsettleDebitsBn.toString(10));
        }
        ...
        if (!gethBalanceBn.equals(blockChainSettledBalanceBn)) {
          ...
          logger.log('=====ddbBalance', oThis.userAddrToDdbBalancesMap[address]);
          logger.log(
            '====Geth balance not matching for',
            address,
            blockChainSettledBalanceBn.toString(10),
            gethBalanceBn.toString(10)
          );
        }
        ...
        if (!pessimisticSettledBalanceBn.equals(blockChainSettledBalanceBn.minus(blockChainUnsettleDebitsBn))) {
          ...
          logger.log(
            '====Pessimistic balance incorrect for',
            address,
            blockChainSettledBalanceBn.toString(10),
            blockChainUnsettleDebitsBn.toString(10),
            pessimisticSettledBalanceBn.toString(10)
          );
        }
      }
    }
    ...
    if (
      ...
      logger.error('===Use below map for correction\n', balanceCorrectionMap);

      debugOptions['balanceCorrectionMap'] = balanceCorrectionMap;
  ...
  async getFinalMismatchAddresses(
    ...
    logger.info('====Result count of non-finalized data query', oThis.esSearchNonFinalizedData.length);

    for (let i = 0; i < oThis.esSearchNonFinalizedData.length; i++) {
      const transactionData = oThis.esSearchNonFinalizedData[i];
      if (!transactionData) {
        continue;
      }
  ...
  async _getNonFinalizedData(queryTimestamp, addresses) {
    ...
    oThis.esSearchNonFinalizedData = esSearchResponse.data[resultType];

    if (oThis.esSearchNonFinalizedData.length === 0) return;
    ...
    while (esSearchResponse.data && esSearchResponse.data.meta && esSearchResponse.data.meta.has_next_page) {
      ...
      oThis.esSearchNonFinalizedData = oThis.esSearchNonFinalizedData.concat(
        paginatedEsSearchResponse.data[resultType]
      );
      esSearchResponse = paginatedEsSearchResponse;
    }

    oThis.esSearchNonFinalizedData = [...new Set(oThis.esSearchNonFinalizedData)]; // Removes duplication.
    logger.log('Es search response ', oThis.esSearchNonFinalizedData);
  }
}
