...
class BalanceVerifier {
  constructor(params) {
    ...
    oThis.nextTimeStamp = oThis.timeStamp + finalizedTimeStamp;
  ...
  async _getDataFromEs() {
    ...
    oThis.esSearchData = esSearchResponse.data[oThis.auxChainId + transactionsString];

    while (esSearchResponse.data.meta.has_next_page) {
      ...
      oThis.esSearchData = oThis.esSearchData.concat(
        paginatedEsSearchResponse.data[oThis.auxChainId + transactionsString]
      );
  ...
  async _getUserAddresses() {
    ...
    if (!oThis.esSearchData.length) return;
  ...
  async _getBalanceFromGeth() {
    ...
    logger.debug('User addresses to geth balance map: ', oThis.userAddrToGethBalanceMap);
  }
  ...
  async _getBalanceFromDdb() {
    ...
    logger.log('User address to ddb balances map :', oThis.userAddrToDdbBalancesMap);
  }
