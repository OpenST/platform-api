  ...
  async _verifyBalance() {
    ...
    for (const address of oThis.finalizedUserAddresses) {
      ...
      if (!oThis.notFinalizedUserAddresses.has(address)) {
        ...
        if (!zeroBigNumber.equals(blockChainUnsettleDebitsBn)) {
          ...
          console.log('====USD non zero for', address, blockChainUnsettleDebitsBn.toString(10));
        }
        ...
        if (!gethBalanceBn.equals(blockChainSettledBalanceBn)) {
          ...
          console.log('=====ddbBalance', oThis.userAddrToDdbBalancesMap[address]);
          console.log(
    ...
    if (
      ...
      let errorObject = responseHelper.error({
        ...
        debug_options: balanceCorrectionMap
      });
