...
class CreateShard {
   ...
   * @private
   */
  _generateShardSuffixes() {
    const oThis = this,
      ShardModel = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'ShardModel');
    for (let i = 0; i < oThis.shardNumbers.length; i++) {
      oThis.shardNumberSuffixes.push(ShardModel.getShardSuffixFromShardNumber(oThis.shardNumbers[i]));
    }
  }
  ...
  async _create() {
    ...
    for (let index = 0; index < oThis.shardNumberSuffixes.length; index++) {
      let modelObj = new Model({ chainId: oThis.chainId, shardNumber: oThis.shardNumberSuffixes[index] });
