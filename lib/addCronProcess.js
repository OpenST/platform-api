...
class AddCronProcess {
  ...
  async _addCronProcess() {
    ...
    if (new CronProcessModel().kinds[oThis.kind] == cronProcessesConstant.executeTransaction) {
      await oThis._createExTxCronProcess();
    } else {
      await oThis._createCronProcess();
    }
    return {};
  }
