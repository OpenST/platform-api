/**
 * Class for inserting fund from token aux funder to exTx workers entry in cron processes table.
 *
 * @module lib/cronProcess/fundByTokenAuxFunder/ToExTxWorkers
 */
 ...
 * @class
 */
class ToExTxWorkers extends CronProcessBase {
  /**
   * Constructor for inserting fund from token aux funder to exTx workers entry in cron processes table.
   *
   * @param {object} params
  ...
  async validateAndSanitize() {
    ...
    if (!CommonValidators.validateNonZeroInteger(oThis.originChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_fbtaf_tetw_1',
    ...
    if (!CommonValidators.validateNonZeroInteger(oThis.auxChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_fbtaf_tetw_2',
          api_error_identifier: 'something_went_wrong',
          debug_options: {}
        })
      );
  ...
  get getCronKind() {
    ...
    oThis.cronKind = cronProcessesConstants.fundByTokenAuxFunderToExTxWorkers;
  }
}

module.exports = ToExTxWorkers;
