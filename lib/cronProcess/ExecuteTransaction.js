...
const rootPrefix = '../..',
  ...
  TransactionCronProcessDetailsModel = require(rootPrefix + '/app/models/mysql/TxCronProcessDetail');
...
class ExecuteTransaction extends CronProcessBase {
  ...
  async validateAndSanitize() {
    ...
    if (Number(oThis.prefetchCount) < 1) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_et_3',
          api_error_identifier: '',
          debug_options: {}
        })
      );
    ...
    if (Number(oThis.sequenceNumber) < 1) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_et_5',
          api_error_identifier: '',
          debug_options: {}
        })
      );
