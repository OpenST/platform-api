...
class AuxWorkflowWorker extends CronProcessBase {
  ...
  async validateAndSanitize() {
    ...
    if (!CommonValidators.validateInteger(oThis.auxChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_aww_1',
    ...
    if (!CommonValidators.validateInteger(oThis.prefetchCount)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_aww_2',
          api_error_identifier: '',
          debug_options: {}
        })
      );
    ...
    if (Number(oThis.prefetchCount) < 1) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_aww_3',
          api_error_identifier: '',
          debug_options: {}
        })
      );
    ...
    if (!CommonValidators.validateInteger(oThis.sequenceNumber)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_aww_4',
          api_error_identifier: '',
          debug_options: {}
        })
      );
    ...
    if (Number(oThis.sequenceNumber) < 1) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_aww_5',
          api_error_identifier: '',
          debug_options: {}
        })
      );
