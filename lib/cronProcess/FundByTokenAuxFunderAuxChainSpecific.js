/**
 * Class for inserting fund from token aux funder aux chain specific entry in cron processes table.
 *
 ...
 * @class
 */
class FundByTokenAuxFunderAuxChainSpecific extends CronProcessBase {
  /**
   * Constructor for inserting fund from token aux funder aux chain specific entry in cron processes table.
   *
   * @param {Object} params
  ...
  async validate() {
    ...
    if (!CommonValidators.validateInteger(oThis.originChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_co_fbtafacs_1',
    ...
    if (!CommonValidators.validateInteger(oThis.auxChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_co_fbtafacs_2',
          api_error_identifier: '',
          debug_options: {}
        })
      );
  ...
  get getCronKind() {
    ...
    oThis.cronKind = cronProcessesConstants.fundByTokenAuxFunderAuxChainSpecific;
  }
}

module.exports = FundByTokenAuxFunderAuxChainSpecific;
