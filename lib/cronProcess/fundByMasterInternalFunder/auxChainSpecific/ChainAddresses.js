 ...
 *
 * @module lib/cronProcess/fundByMasterInternalFunder/auxChainSpecific/ChainAddresses
 */
 ...
 * @class
 */
class ChainAddresses extends CronProcessBase {
  /**
  ...
  async validateAndSanitize() {
    ...
    if (!CommonValidators.validateNonZeroInteger(oThis.originChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_fbmif_acs_ca_1',
    ...
    if (!CommonValidators.validateNonZeroInteger(oThis.auxChainId)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_cp_fbmif_acs_ca_2',
          api_error_identifier: 'something_went_wrong',
          debug_options: {}
        })
      );
