...
class NonceHelperKlass {
   ...
   */
  static getMinedTransactionCount(chainType, address, gethProviders) {
    try {
      ...
      for (let i = allGethNodes.length - 1; i >= 0; i--) {
        ...
        const web3Provider = CustomWebProvider.getInstance(gethURL, chainType);
   ...
   * @return {promise<result>}
   */
  static async getAllQueuedTransaction(chainType, gethProviders) {
    try {
   ...
   * @return {promise<result>}
   */
  static async clearAllMissingNonce(chainType, scope, clearCallback, gethProviders) {
    try {
      const allQueuedTransaction = await NonceHelperKlass.getAllQueuedTransaction(chainType, gethProviders);
   ...
   * @return {promise<result>}
   */
  static async clearMissingNonce(address, chainType, pendingTransactions, scope, clearCallback, gethProviders) {
    if (!clearCallback) {
      return Promise.resolve(
        responseHelper.error({
  ...
  static async getUnminedTransactionsFromNode(wsWeb3Provider, rpcGethURL) {
    return new Promise(async function(onResolve, onReject) {
      try {
        ...
        //Format the error
        if (rpcGethURL) {
          const rpcWeb3Provider = CustomWebProvider.getInstance(rpcGethURL, wsWeb3Provider.chainType),
