...
class SetPriceInPriceOracleContract {
  ...
  async _fetchAddresses() {
    ...
    const fetchStakeCurrencyDetailsRsp = await new StakeCurrencyBySymbolCache({
      stakeCurrencySymbols: [oThis.baseCurrency]
    }).fetch();

    if (fetchStakeCurrencyDetailsRsp.isFailure()) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_upp_spipo_3',
          api_error_identifier: 'something_went_wrong'
        })
      );
    }

    let stakeCurrencyDetails = fetchStakeCurrencyDetailsRsp.data[oThis.baseCurrency];
    oThis.auxPriceOracleContractAddress = stakeCurrencyDetails.priceOracleContractAddress;
  }
  ...
  async _insertPricePointInTable(transactionHash) {
    ...
    let response = await stakeCurrencyBySymbolCache.fetch(),
      stakeCurrencyId = response.data[oThis.currentErc20Value.baseCurrency].id;
    ...
    const insertResponse = await new CurrencyConversionRateModel()
      .insert({
        ...
        base_currency: conversionRateConstants.invertedBaseCurrencies[oThis.currentErc20Value.baseCurrency], // TODO: @Santhosh - Remove this in 2.1-cleanup branch
