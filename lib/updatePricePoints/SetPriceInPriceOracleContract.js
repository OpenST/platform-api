...
class SetPriceInPriceOracleContract {
  ...
  async _fetchAddresses() {
    ...
    if (oThis.baseCurrency === stakeCurrencyConstants.OST) {
    ...
    } else if (oThis.baseCurrency === stakeCurrencyConstants.USDC) {
      oThis.auxPriceOracleContractAddress =
        chainAddressesRsp.data[chainAddressConstants.auxUsdcToUsdPriceOracleContractKind].address;
    } else {
      throw new Error(`Invalid baseCurrency ${oThis.baseCurrency}`);
    }
  }
  ...
  async _insertPricePointInTable(transactionHash) {
    ...
    const insertResponse = await new CurrencyConversionRateModel()
      .insert({
        ...
        base_currency: conversionRateConstants.invertedBaseCurrencies[oThis.baseCurrency], // TODO: @Santhosh - Remove this in 2.1-cleanup branch
