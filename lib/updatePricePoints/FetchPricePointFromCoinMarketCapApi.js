...
const s3FileUrl = coreConstants.SA_S3_PRICEPOINTS_BUCKET + '/coinmarketcap/api/v2/convert';
...
class FetchPricePointFromCoinMarketCapApi {
  ...
  async _fetchPricePointFromCoinMarketCapApi() {
    ...
    try {
      ...
      if (!currentErc20ValueRsp || !currentErc20ValueRsp[oThis.baseCurrency]) {
        logger.error('Error in fetching data from coinMarketCap: ', currentErc20ValueRsp);
        oThis.setOldValuesInContract = true;

        return;
      }
      const pricePoint = currentErc20ValueRsp[oThis.baseCurrency][oThis.quoteCurrency.toLowerCase()];
