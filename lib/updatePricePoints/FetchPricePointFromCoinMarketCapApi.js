...
const symbolToNameMap = {
  [conversionRateConstants.OST]: 'simple-token',
  [conversionRateConstants.USDC]: 'usd-coin'
};
...
class FetchPricePointFromCoinMarketCapApi {
  ...
  async _fetchPricePointFromCoinMarketCapApi() {
    const oThis = this;
    const url = exchangeUrl + symbolToNameMap[oThis.baseCurrency] + '?convert=' + oThis.quoteCurrency;
    ...
    try {
      ...
      if (!currentErc20ValueRsp || currentErc20ValueRsp.symbol !== 'OST') {
