...
class MySQLQueryBuilderKlass {
  ...
  insert(insertFields, insertOptions) {
    ...
    let insertColumns = Object.keys(insertFields),
  ...
  generate() {
    ...
    if (oThis.queryType === 'SELECT') {
      ...
      return oThis._generateDescribe();
  ...
  /**
   * For fetching column name: SHOW COLUMNS FROM table_name
   *
   * @return {MySQLQueryBuilderKlass}
   */
  showColumns() {
    const oThis = this;
  ...
  /**
   * Generate actual query for showColumns function
   * @returns {*|result}
   * @private
   */
