...
class ByPersonalSign extends Base {
   ...
   * @param {String} params.inputParams.wallet_address -
   * @param {String} params.inputParams.api_signer_address - address using whose private key was signature generated
  ...
  async _validateParams() {
    ...
    if (!CommonValidators.validateEthAddress(oThis.inputParams['api_signer_address'])) {
      paramErrors.push('invalid_api_signer_address');
    } else {
      oThis.personalSignAddress = oThis.inputParams['api_signer_address'].toLowerCase();
    }
    ...
    if (oThis.userDeviceData.personalSignAddress !== oThis.personalSignAddress) {
      return oThis._validationError('l_vas_bps_5', ['invalid_api_signer_address']);
    }
