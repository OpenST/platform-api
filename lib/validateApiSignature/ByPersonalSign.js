...
class AuthenticateApiByPersonalSign extends Base {
  ...
  async _validateParams() {
    const oThis = this,
      paramErrors = [];
    logger.debug('apiAuthentication: ByPersonalSignStep: 1');
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 2');
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 3');
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 4');
    ...
    if (response.isFailure() || !response.data.user) {
      return oThis._validationError('l_vas_bps_4', ['invalid_api_key_for_not_deployed_token'], {
        tokenShardDetails: response.data
      });
    }

    logger.debug('apiAuthentication: ByPersonalSignStep: 5');
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 6');
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 7');
    ...
    if (oThis.userDeviceData.status === deviceConstants.revokedStatus) {
      return oThis._validationError('l_vas_bps_9', ['invalid_api_key_for_revoked_device'], {
        userDeviceData: oThis.userDeviceData
      });
    }
    ...
    if (oThis.userDeviceData.personalSignAddress !== oThis.apiKeyPersonalSignAddress) {
      return oThis._validationError('l_vas_bps_10', ['invalid_api_key_for_signer_address'], {
        apiKeyPersonalSignAddress: oThis.apiKeyPersonalSignAddress,
        userDeviceData: oThis.userDeviceData
      });
    }
  ...
  async _validateSignature() {
    ...
    logger.debug('apiAuthentication: ByPersonalSignStep: 8');
    ...
    if (verifySignRsp.isFailure()) {
      return oThis._validationError('l_vas_bps_11', ['invalid_api_signature']);
    }

    logger.debug('apiAuthentication: ByPersonalSignStep: 9');
