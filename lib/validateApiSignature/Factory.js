...
class Factory {
  ...
  async _asyncPerform() {
    ...
    switch (oThis.inputParams['signature_kind']) {
      ...
      default:
        throw `oThis.inputParams['signature_kind'] ${oThis.inputParams['signature_kind']}`;
    }
  }
  ...
  _validateParams() {
    ...
    if (!apiSignature.supportedKinds.includes(oThis.inputParams['signature_kind'])) {
    ...
    if (oThis.requestMethod === apiSignature.getRequestKind) {
      apiConfigs = apiAuthentication.getRequestsDataExtractionReges;
    } else if (oThis.requestMethod === apiSignature.postRequestKind) {
      apiConfigs = apiAuthentication.postRequestsDataExtractionReges;
    } else {
    ...
    if (!matchData.supportedSignatureKinds.includes(oThis.inputParams['signature_kind'])) {
      return oThis._validationError('l_vas_f_4', ['unsupported_signature_kind']);
    }
