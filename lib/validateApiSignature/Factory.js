...
class Factory {
  ...
  async _asyncPerform() {
    ...
    await oThis._checkIfReplayAttack();
    ...
    switch (oThis.inputParams['api_signature_kind']) {
      ...
      case apiSignature.personalSignKind:
        ...
        break;
      default:
        throw `unreachable code -> validations already exist above this -> ${oThis.inputParams['api_signature_kind']}`;
    }

    logger.debug('apiAuthentication: 6');
  ...
  async _checkIfReplayAttack() {
    ...
    if (oThis.requestMethod !== apiSignature.postRequestKind) return;

    let replayAttackCache = new ReplayAttackCache({
      signature: oThis.inputParams.api_signature
    });

    let response = await replayAttackCache.fetch();
  ...
  _matchAndExtract(apiConfigs) {
    ...
    for (let index = 0; index < apiConfigs.length; index++) {
      let urlApiConfig = apiConfigs[index];
      ...
      if (
        ...
        matchObj.length === urlApiConfig.regExMatches.length
      ) {
        matchUrl = {
