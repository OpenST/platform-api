...
class Factory {
  ...
  async _asyncPerform() {
    ...
    const validationResponse = await oThis._validateParams();

    if (validationResponse.isFailure()) {
      return validationResponse;
    }

    const matchData = validationResponse.data;

    logger.debug('apiAuthentication: 2');

    await oThis._checkIfReplayAttack(matchData);
    ...
    switch (oThis.inputParams.api_signature_kind) {
      ...
      case apiSignature.personalSignKind:
        ...
        break;
      case apiSignature.noAuthKind:
        // Do nothing. Consider it valid
        response = responseHelper.successWithData({
          apiSignatureKind: apiSignature.noAuthKind,
          tokenId: oThis.inputParams.tokenId
        });
        break;
      default:
        throw new Error(
    ...
    if (response.isSuccess()) {
      response.data.appValidatedApiName = matchData.apiName;
    }
