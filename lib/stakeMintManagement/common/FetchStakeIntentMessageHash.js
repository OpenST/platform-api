...
/*
 *
 *  @module lib/stakeMintManagement/common/FetchStakeIntentMessageHash
 */
    ...
    if (resp.isSuccess()) {
      return Promise.resolve(
        responseHelper.successWithData({
          taskDone: 1,
          taskResponseData: { messageHash: resp.data.messageHash }
        })
      );
    } else {
        ...
        if (event.name === 'StakeIntentDeclared') {
          for (let i = 0; i < event.events.length; i++) {
            ...
            if ((eventData.name = '_messageHash')) {
              ...
              return Promise.resolve(responseHelper.successWithData({ messageHash: messageHash }));
            }
          }
        }
      }
