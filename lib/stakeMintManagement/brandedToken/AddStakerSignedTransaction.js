...
class AddStakerSignedTransaction extends Base {
  ...
  async _checkTransactionFinalized() {
    ...
    let isTrxFinalized = false,
      gtdKlass = new GetDetailsFromDDB({ chainId: oThis.originChainId, transactionHashes: [oThis.transactionHash] }),
      dbResponse = await gtdKlass.perform();

    let ddbTransaction = null;
    if (dbResponse.isSuccess()) {
      ddbTransaction = dbResponse.data[oThis.transactionHash];
    }

    // Transaction is already present
    if (ddbTransaction) {
      let txBlock = ddbTransaction.blockNumber,
        finalizedBlock = await oThis._getLastFinalizedBlock();

      isTrxFinalized = txBlock <= finalizedBlock;
    }

    return Promise.resolve(isTrxFinalized);
  }
  ...
  async _insertPendingTransaction() {
    ...
    return createPendingTransaction.insertPendingTransaction(txOptions, oThis.transactionHash, oThis.payloadDetails);
  }
}
