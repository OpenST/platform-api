...
class Transaction {
  ...
  _validateTransactionStatusesWithWebHook(transactionsResponse, payload) {
    ...
    for (let ind = 0; ind < transactionsResponse.length; ind++) {
      ...
      // If transaction status for webhook kind is not valid
      if (!webhookTopicToEntityStatusConstants.isEntityValid(payload.webhookKind, txObj.status)) {
        isValidStatus = false;
      }
    }
    ...
    if (isValidStatus) {
      ...
      return responseHelper.error({
        ...
        debug_options: {
          msg: 'Webhook Kind to send is not in sync with transaction data.',
