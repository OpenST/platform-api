...
class Transaction {
  ...
  async _fetchTxDetails(ic, payload, esSearchResponse) {
    ...
    return oThis._validateTransactionStatusesWithWebHook(transactionsResponse.data, payload);
  }

  /**
   * Validate transactions status with Webhook to send out.
   *
   ...
   * @private
   */
  _validateTransactionStatusesWithWebHook(transactionsResponse, payload){
    const oThis = this;

    let isValidStatus = true;
    for(let ind=0;ind<transactionsResponse.length;ind++){
      const txObj = transactionsResponse[ind];
      // If webhook kind is success
      if(payload.webhookKind == webhookSubscriptionsConstants.transactionsSuccessTopic &&
        txObj.status != pendingTransactionConstants.successStatus){
        isValidStatus = false;
      } else if(payload.webhookKind == webhookSubscriptionsConstants.transactionsFailureTopic &&
        txObj.status != pendingTransactionConstants.failedStatus){
        isValidStatus = false;
      } else if(payload.webhookKind == webhookSubscriptionsConstants.transactionsMinedTopic &&
        txObj.status != pendingTransactionConstants.minedStatus){
        isValidStatus = false;
      } else if(payload.webhookKind == webhookSubscriptionsConstants.transactionsInitiateTopic &&
        ![pendingTransactionConstants.createdStatus, pendingTransactionConstants.submittedStatus].includes(txObj.status)){
        isValidStatus = false;
      }
    }

    // If webhook to send is not in sync with transaction status
    if(isValidStatus){
      return responseHelper.successWithData({
        entityResultType: resultType.transaction,
        rawEntity: transactionsResponse[0]
      });
    } else {
      return responseHelper.error({
        internal_error_identifier: 'l_w_d_t_2',
        api_error_identifier: 'something_went_wrong',
        debug_options: {msg: "Webhook Kind to send is not in sync with transaction data.",
          transactionsResponse: transactionsResponse, webhookPayload: payload}
      });
    }
  }
}
