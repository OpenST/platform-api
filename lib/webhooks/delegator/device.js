...
class Device {
  ...
  _filteredEntityToSend(webhookKind, rawEntity) {
    ...
    if (allowedEntityStatuses.indexOf(rawEntity.status) <= -1) {
      ...
      if (webhookKind == webhookSubscriptionsConstants.devicesRecoveryInitiateTopic) {
        // Check if status has progressed
        if ([deviceConstants.authorizedStatus, deviceConstants.revokingStatus].indexOf(rawEntity.status) <= -1) {
          return false;
        } else {
          // Entity can be sent, now filter out data which is not allowed to be sent now
          Object.assign(rawEntity, { status: deviceConstants.recoveringStatus });
        }
      } else {
        return false;
      }
