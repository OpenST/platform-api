...
const rootPrefix = '../../..',
  ...
  webhookTopicToEntityStatusConstants = require(rootPrefix +
    '/lib/globalConstant/webhookSubscriptionsToEntityStatuses');
...
class Factory {
  ...
  async _asyncPerform(payload) {
    ...
    const entityResponse = await oThis._createEntity(payload, ic);

    // Validate entity to be sent outside
    console.log('Entity Response: ', entityResponse);
    if (entityResponse.isSuccess()) {
      await oThis._validateEntityStatus(payload, entityResponse.data.rawEntity);
    }
  ...
  /**
   * Validate entity status which can be sent as webhook data
   *
   * @param payload
   * @param rawEntity
   ...
   * @private
   */
  async _validateEntityStatus(payload, rawEntity) {
    const oThis = this;

    // Data not to be validated for price points
    if (
      [
        webhookSubscriptionsConstants.usdPricePointUpdatedTopic,
        webhookSubscriptionsConstants.gbpPricePointUpdatedTopic,
        webhookSubscriptionsConstants.eurPricePointUpdatedTopic
      ].includes(payload.webhookKind)
    ) {
      return;
    }
    console.log('Validate Entity Response for status: ', rawEntity.status, payload.webhookKind);
    if (!webhookTopicToEntityStatusConstants.isEntityValid(payload.webhookKind, rawEntity.status)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_w_d_f_2',
