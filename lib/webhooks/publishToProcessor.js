...
class PublishToProcessor {
  ...
  async _sendIfSubscribed(msgPayload, chainId) {
    ...
    if (activeWebhookKindsForCurrentClient.data[webhookKindInt]) {
      ...
      let extraData = null,
        pendingWebhookId = msgPayload.pendingWebhookId || null,
        webhookStatus = pendingWebhookConstants.invertedStatuses[pendingWebhookConstants.queuedStatus];
      if (entityResponse.isFailure()) {
        ...
        webhookStatus = pendingWebhookConstants.invertedStatuses[pendingWebhookConstants.dataBuildingFailedStatus];
      } else {
        extraData = JSON.stringify({
          webhookEndpointUuid: activeWebhookKindsForCurrentClient.data[webhookKindInt],
      ...
      pendingWebhookId = await oThis._insertUpdatePendingWebhook(pendingWebhooksParams, pendingWebhookId);
   ...
   * @private
   */
  async _insertUpdatePendingWebhook(pendingWebhooksParams, pendingWebhookId) {
    const oThis = this;

    // If webhook id is already present, then it would be update.
    if (pendingWebhookId) {
      await new PendingWebhookModel()
        .update({ extra_data: pendingWebhooksParams.extraData, status: pendingWebhooksParams.status })
        .where({ id: pendingWebhookId })
        .fire();

      // Clear old pending webhooks cache.
      await new PendingWebhooksCache({ pendingWebhookId: pendingWebhookId }).clear();
    } else {
