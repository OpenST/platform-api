...
class PublishToProcessor {
  ...
  async _sendIfSubscribed(msgPayload, chainId) {
    ...
    if (activeWebhookKindsForCurrentClient.data[webhookKindInt]) {
      ...
      if (entityResponse.isFailure()) {
        ...
        pendingWebhooksParams.nextRetryAt = Math.floor(Date.now() / 1000) + 1;
      } else {
        pendingWebhooksParams.extraData = JSON.stringify({
          webhookEndpointUuid: activeWebhookKindsForCurrentClient.data[webhookKindInt],
  ...
  async _insertUpdatePendingWebhook(pendingWebhooksParams, pendingWebhookId, entityResponse) {
    ...
    if (pendingWebhookId) {
      let nextRetryAt = pendingWebhooksParams.nextRetryAt;
      ...
      if (cacheResp.isSuccess()) {
        if (cacheResp.data.retryCount + 1 > pendingWebhookConstants.maxDataBuildingFailedRetryCount) {
          ...
          // Overwrite data building failed retry time
          if (cacheResp.data.retryCount + 1 > 20) {
            // If 20 retries has been done, overwrite next retry to 5 seconds
            nextRetryAt = Math.floor(Date.now() / 1000) + 5;
          } else if (cacheResp.data.retryCount + 1 > 10) {
            // If 10 retries has been done, overwrite next retry to 3 seconds
            nextRetryAt = Math.floor(Date.now() / 1000) + 3;
          }
        }
      }
