...
class PublishWebhook {
  ...
  async _getEndpointDetails() {
    ...
    console.log('----webhookEndpointUuids-----------', webhookEndpointUuids);
  ...
  async _markPendingWebhookFailed() {
    ...
    if (oThis.pendingWebhook.retryCount >= pendingWebhookConstants.maxRetryCount) {
      ...
      let retryDetails = oThis._getRetryDetails();
  ...
  _getRetryDetails() {
    ...
    return {
      ...
      nextRetryAt: oThis.pendingWebhook.nextRetryAt + pendingWebhookConstants.getNextRetryAtDelta(retryCount)
    };
  }
