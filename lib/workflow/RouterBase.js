...
class WorkflowRouterBase {
  ...
  async _insertWorkflowStep(nextStepKind, nextStepStatus) {
    ...
    // TODO: Temporary fix for multiple workflow step entries. Remove it @Santhosh
    let response = await new WorkflowStepsModel()
      .select('id')
      .where(['WORKFLOW_ID = ? AND KIND = ? AND STATUS IS NOT NULL', oThis.workflowId, nextStepKind])
      .fire();
