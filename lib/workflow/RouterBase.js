...
class WorkflowRouterBase {
  ...
  perform() {
    ...
    return oThis.asyncPerform().catch(async function(error) {
      ...
      logger.error('lib/workflow/RouterBase::perform::catch');
      logger.error(error);
  ...
  async _performStepIfReadyToStart() {
    ...
    const response = await oThis._performStep();
  ...
  _decideChainId() {
    ...
    switch (oThis.stepKind) {
      ...
      case workflowStepConstants.updateCommittedStateRootInfo:
        ...
        break;
      default:
        logger.error(`Unsupported workflow step: ${oThis.stepKind}`);
      // throw new Error(`Unsupported workflow step: ${oThis.stepKind}`);
    }
  ...
  async _insertAndPublishFor(nextStep) {
    ...
    if (oThis._rabbitmqKind == rabbitmqConstant.auxRabbitmqKind) {
  ...
  async _handleCaughtErrors(debugParams = null) {
    ...
    await emailNotifier.perform(
      'WorkflowFailed',
      `*** Workflow with id ${oThis.workflowId} failed!`,
      {},
      { workflowId: oThis.workflowId }
    );
  ...
  async handleFailure() {
    ...
    await emailNotifier.perform('WorkflowFailed', `*** Workflow with id ${oThis.workflowId} failed!`, {}, {});
