...
class LogoutSessionsRouter extends AuxWorkflowRouterBase {
   ...
   * @param {string} status
   *
   * @returns {{chainId: string, payload: {webhookKind: string, clientId: string, tokenId: string, userId: string}}}
   */
  preProcessorWebhookDetails(status) {
    ...
    return {
      ...
      payload: {
        webhookKind: status
          ? webhookSubscriptionsConstants.sessionsLogoutAllSuccessTopic
          : webhookSubscriptionsConstants.sessionsLogoutAllFailureTopic,
