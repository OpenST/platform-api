 ...
 *
 * @module lib/executeTransactionManagement/GenerateExTxWorker
 */
const rootPrefix = '../..',
...
class GenerateExTxWorker {
  ...
  async _assignTxWorkersToProcesses() {
    ...
    if (!cronProcesses.length)
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_etm_getw_2',
          api_error_identifier: 'something_went_wrong',
          debug_options: { err: 'No running or stopped cron process for ex tx.' }
        })
      );
    ...
    if (!cronProcessIds.length)
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_etm_getw_3',
          api_error_identifier: 'something_went_wrong',
          debug_options: { err: 'All ex tx processes inactive.' }
        })
      );

    let txCronProcessDetails = await new TxCronProcessDetailsModel()
      .select('*')
      .where(['cron_process_id IN (?) AND chain_id=?', cronProcessIds, oThis.auxChainId])
      .fire();
