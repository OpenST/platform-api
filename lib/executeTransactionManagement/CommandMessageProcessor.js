...
class CommandMessageProcessor {
  /**
   * @param {Object} params
   * @param {Integer} params.chainId - aux ChainId
  ...
  async asyncPerform() {
    ...
    // Get Queue topic suffix for all processIds present on current chainId.
  ...
  async _executeMarkBlockingToOriginalStatusCommand() {
    ...
    // Return if the worker does not have "blockingProperty".
    if (
      !(
    ...
    for (let i = 0; i < siblingHoldWorkerProcesses.length; i++) {
      let workerProcess = siblingHoldWorkerProcesses[i],
        ...
        topicName = kwcConstant.commandMessageTopicName(oThis.chainId, queueNameSuffix);
  ...
  async _executeGoOnHoldCommand() {
    ...
    console.log(
      'tokenExtxWorkerProcessesConstants.invertedProperties',
      tokenExtxWorkerProcessesConstants.invertedProperties
    );
