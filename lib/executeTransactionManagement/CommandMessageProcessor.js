...
class CommandMessageProcessor {
  ...
  async _getNotificationObject() {
    ...
    oThis.ostNotification = await rabbitmqProvider.getInstance(rabbitmqConstants.auxRabbitmqKind, {
  ...
  async _executeMarkBlockingToOriginalStatusCommand() {
    ...
    logger.step('Starting _executeMarkBlockingToOriginalStatusCommand for -> \n', oThis.commandMessage);
