...
class Base {
  ...
  async perform() {
    ...
    let sanitizedTransferToAddresses = [];
    for (let i = 0; i < oThis.transferToAddresses.length; i++) {
      sanitizedTransferToAddresses.push(basicHelper.sanitizeAddress(oThis.transferToAddresses[i]));
    }
    ...
    return responseHelper.successWithData({
      ...
      gas: oThis.gas,
      transferToAddresses: sanitizedTransferToAddresses
    });
  }
