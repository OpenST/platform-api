...
class PricerRule extends Base {
  ...
  async _validateStakeCurrencyToUsdPrice() {
    ...
    let decimals = pricePointsGetRsp.data.decimals,
      stakeCurrencyInUsdFromCache =
        pricePointsGetRsp.data[oThis.stakeCurrencyData['symbol']][conversionRatesConstants.USD],
  ...
  _convertToBtWei(amountinUsdWei) {
    ...
    let btInNormal = basicHelper.convertWeiToNormal(btInWei); // removing the 10^18 multiplied for USD wei conversion

    return basicHelper.convertToLowerUnit(btInNormal, oThis.stakeCurrencyData['decimal']);
  }
