...
class PricerRule extends Base {
  ...
  async _validateExecutableData() {
    ...
    await oThis._validateOstToUsdPrice();
  }
   ...
   * @private
   */
  async _validateOstToUsdPrice() {
    const oThis = this;

    if (!CommonValidators.validateNonZeroWeiValue(oThis.ostToUsdInWeiFromRawCallData)) {
    ...
    let decimals = pricePointsGetRsp.data.decimals,
      ostInUsdFromCache = pricePointsGetRsp.data[conversionRatesConstants.OST][conversionRatesConstants.USD],
      bnOstInUsdInWeiFromCache = new BigNumber(ostInUsdFromCache).mul(new BigNumber(10).toPower(decimals)),
      acceptedMargin = contractConstants.acceptanceMargin;

    let diff = bnOstInUsdInWeiFromCache.minus(oThis.bNostInUsdinWei);
    ...
    if (diff.gt(acceptedMargin)) {
      return oThis._validationError('l_etm_ped_pr_7', ['invalid_raw_calldata_ost_to_usd_value'], {
        acceptedMargin: acceptedMargin,
        diff: diff,
        ostToUsdInWeiFromRawCallData: oThis.ostToUsdInWeiFromRawCallData,
        bnOstInUsdInWeiFromCache: bnOstInUsdInWeiFromCache.toString(10)
      });
    }
  }
