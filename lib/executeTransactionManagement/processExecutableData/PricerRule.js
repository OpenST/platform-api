...
class PricerRule extends Base {
  ...
  async _validateStakeCurrencyToUsdPrice() {
    ...
    let stakeCurrencyData = stakeCurrencyCacheResponse.data[oThis.stakeCurrencyId];
    ...
    let decimals = pricePointsGetRsp.data.decimals,
      stakeCurrencyInUsdFromCache = pricePointsGetRsp.data[stakeCurrencyData['symbol']][conversionRatesConstants.USD],
  ...
  async _setPessimisticDebitAmount() {
    ...
    oThis.bNStakeCurrencyInUsd = basicHelper.convertWeiToNormal(oThis.bNStakeCurrencyInUsdWei);
  ...
  _convertToBtWei(amountinUsdWei) {
    ...
    return new BigNumber(amountinUsdWei).mul(oThis.bNStakeCurrencyInBt).div(oThis.bNStakeCurrencyInUsd);
  }
