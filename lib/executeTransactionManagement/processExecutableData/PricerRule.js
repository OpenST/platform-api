...
class PricerRule extends Base {
  ...
  async _validateExecutableData() {
    ...
    if (
      ...
      oThis.transferToAddresses.includes(oThis.tokenHolderAddress)
    ) {
      return oThis._validationError('l_etm_ped_pr_2', ['invalid_raw_calldata_parameter_address'], {
        rawCallDataParameters: oThis.rawCallDataParams
      });
    }

    if (!CommonValidators.validateWeiAmountArray(oThis.transferAmountsinUsdWei)) {
      return oThis._validationError('l_etm_ped_pr_3', ['invalid_raw_calldata_parameter_amount'], {
        rawCallDataParameters: oThis.rawCallDataParams
      });
    }
  }
