...
class AssociateWorker {
  ...
  async _getAndValidateAvailableWorkers() {
    ...
    oThis.processIds = [...new Set(oThis.processIds.filter((x) => !oThis.associatedProcessIds.has(x)))];

    if (oThis.processIds.length == 0) {
      logger.info('There are no valid process ids: ', oThis.processIds);
      process.exit(1);
    }
    logger.info('Valid processIds are : ', oThis.processIds);
  }
  ...
  async _getAndValidateProcessDetails() {
    ...
    let dbRows = await txCronProcessDetailsObject
      .select('*')
      .where(['chain_id = ? AND id IN (?)', oThis.auxChainId, oThis.processIds])
      .fire();
    ...
    for (let i = 0; i < dbRows.length; i++) {
      ...
      //oThis.cronProcessIdsToDetailsMap[dbRow.id].push(dbRow);

      chainIdsOnCurrentChain.add(parseInt(dbRow.id));
    }
    ...
    oThis.processIds = [...new Set(oThis.processIds.filter((x) => chainIdsOnCurrentChain.has(x)))];

    logger.info('Valid processIds are after filtering processIds for current chain are : ', oThis.processIds);
  }
  ...
  async _associateAvailableWorker() {
    const oThis = this,
      noOfWorkersToBeAssociated = Math.min(oThis.processIds.length, oThis.availableExTxWorkersId.length);
