...
class AssociateWorker {
  ...
  async _fundExTxWorkers() {
    ...
    logger.debug('===== exTxWorkerAddresses =====', exTxWorkerAddresses);
    logger.debug('===== fundResponse =====', fundResponse);
    if (fundResponse.isFailure()) {
      return Promise.reject(fundResponse);
    }
    let exTxWorkerAddressToTxHash = fundResponse.data.taskResponseData.transactionHashes;

    //Check if all workers have received stprime fund

    for (let i = 0; i < exTxWorkerAddresses.length; i++) {
      let addr = exTxWorkerAddresses[i];
      if (!exTxWorkerAddressToTxHash[addr]) {
        return Promise.reject('Fund(STPrime) was not transfered to worker address - ' + addr);
      }
    }
    return true;
  }
