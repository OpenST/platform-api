...
class ChainCacheManagementBase {
   ...
   * Set data in cache.
   *
   * @param dataToSet: data to set in cache
    ...
    return setCacheFunction().then(function(cacheSetResponse) {
      if (cacheSetResponse.isFailure()) {
        const errorObject = responseHelper.error({
          internal_error_identifier: 'cache_not_set:l_cm_c_b_1',
        ...
        logger.error('l_cm_c_b_1', 'Error in setting cache.', cacheSetResponse.getDebugData());
      }
    });
  }
