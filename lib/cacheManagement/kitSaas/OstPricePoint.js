...
class OstPricePointsCache extends BaseCacheManagement {
  ...
  async _fetchDataFromSource() {
    ...
    while (missingBaseCurrenciesArray.length > 0 && retryCount) {
    ...
    if (missingBaseCurrenciesArray.length > 0) {
   ...
   * @private
   */
  _extractMissingBaseCurrencies(pricePointsData) {
    let missingCurrencies = [];

    for (let currency in conversionRatesConstants.invertedBaseCurrencies) {
      if (!pricePointsData.hasOwnProperty(currency)) {
