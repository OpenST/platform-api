...
class ChainAddress extends CacheManagementKitSaasBase {
  ...
  async _fetchDataFromSource() {
    const oThis = this,
      activeStatus = chainAddressConstants.invertedStatuses[chainAddressConstants.activeStatus];
    ...
    for (let i = 0; i < dbRows.length; i++) {
      let dbRow = dbRows[i],
        addressKindStr = chainAddressConstants.kinds[dbRow.kind.toString()],
        deployedChainKind = dbRow.deployed_chain_kind
          ? chainAddressConstants.deployedChainKinds[dbRow.deployed_chain_kind.toString()]
      ...
      if (chainAddressConstants.nonUniqueKinds.indexOf(addressKindStr) > -1) {
        if (!addressesHash[addressKindStr]) {
          addressesHash[addressKindStr] = [];
        }
        addressesHash[addressKindStr].push(formattedData);
      } else {
        addressesHash[addressKindStr] = formattedData;
      }
    }
