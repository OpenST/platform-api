...
class ValidDomainByTokenId extends BaseKitSaasMultiCacheManagement {
   ...
   * Set cache keys.
   *
   * @sets oThis.saasCacheKeys, oThis.kitCacheKeys
   *
   * @private
   */
  _setCacheKeys() {
    ...
    for (let index = 0; index < oThis.tokenIds.length; index++) {
      ...
      oThis.kitCacheKeys[oThis._kitCacheKeyPrefix() + 'cm_ksm_vdbt_' + oThis.tokenIds[index]] = oThis.tokenIds[index];
    }
  }
  ...
  _setCacheExpiry() {
    ...
    oThis.cacheExpiry = 24 * 60 * 60 * 3; // 3 day;
  }
