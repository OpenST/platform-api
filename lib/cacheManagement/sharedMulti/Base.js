...
class CacheManagementSharedMultiCache {
  ...
  _setCache(key, dataToSet) {
    ...
    const setCacheFunction = function(key, value) {
    ...
    setCacheFunction(key, dataToSet).then((cacheSetResponse) => {
      if (cacheSetResponse.isFailure()) {
        basicHelper.notify('l_cm_sm_b_1', 'Error in setting cache.', {}, cacheSetResponse.getDebugData());
        logger.error('l_cm_sm_b_2', 'Error in setting cache.', cacheSetResponse.getDebugData());
      }
    });
  }
