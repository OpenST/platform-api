...
class CacheManagementChainMultiBase {
  ...
  async fetch() {
    ...
    if (basicHelper.isEmptyObject(oThis.cacheKeys)) {
      console.trace('Empty cache keys object in lib/cacheManagement/chainMulti/Base');
    } else {
      logger.debug('CACHE-FETCH## lib/cacheManagement/chainMulti/Base cache key: ', oThis.cacheKeys);
    }
    ...
    while (data.cacheMiss.length > 0) {
      ...
      for (let index = 0; index < cacheMissData.length; index++) {
        ...
        if (!basicHelper.isEmptyObject(dataToSet)) {
          dataToSet = oThis.validateDataToSet(dataToSet);
        }
  ...
  async clear() {
    ...
    for (let index = 0; index < Object.keys(oThis.cacheKeys).length; index++) {
      ...
      logger.debug('CACHE-CLEAR## lib/cacheManagement/chainMulti/Base: ', cacheKey);
    ...
    return setCacheFunction(key, dataToSet).then(function(cacheSetResponse) {
      if (cacheSetResponse.isFailure()) {
        const errorObject = responseHelper.error({
          internal_error_identifier: 'cache_not_set:l_cm_cm_b_1',
