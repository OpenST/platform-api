  ...
  _setCache(key, dataToSet) {
    ...
    setCacheFunction(key, dataToSet).then(function(cacheSetResponse) {
      if (cacheSetResponse.isFailure()) {
        basicHelper.notify('l_cm_cm_b_1', 'Error in setting cache.', {}, cacheSetResponse.getDebugData());
        logger.error('l_cm_cm_b_2', 'Error in setting cache.', cacheSetResponse.getDebugData());
      }
    });
  }
