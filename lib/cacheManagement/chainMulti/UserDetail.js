 ...
 *
 * @module lib/cacheManagement/chainMulti/UserDetailCache
 */
...
class UserDetailCache extends CacheManagementChainMultiBase {
  /**
   * Constructor
   *
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.tokenHolderAddresses.length; i++) {
      oThis.cacheKeys[
        oThis._cacheKeyPrefix() + 'cm_ud_' + oThis.tokenId + '_' + oThis.tokenHolderAddresses[i].toLowerCase()
      ] =
  ...
  async fetchDataFromSource(cacheMissAddresses) {
    ...
    if (!oThis.tokenId) {
      logger.trace('Token id was not passed.');
  ...
  validateDataToSet(dataToSet) {
    ...
    if (dataToSet.tokenId == oThis.tokenId) {
      return dataToSet;
    } else {
      return {};
    }
...
InstanceComposer.registerAsShadowableClass(UserDetailCache, coreConstants.icNameSpace, 'UserDetailCache');
