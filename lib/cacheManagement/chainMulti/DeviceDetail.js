...
class DeviceDetail extends CacheManagementChainMultiBase {
   ...
   * @param {String} params.walletAddresses  - wallet Address
  ...
  async fetchDataFromSource() {
    ...
    if (!oThis.shardNumber) {
      let tokenCache = new TokenCache({ clientId: oThis.clientId }),
        tokenCacheResponse = await tokenCache.fetch(),
        tokenId = tokenCacheResponse.data.id,
        TokenUSerDetailsCache = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'TokenUserDetailsCache'),
        tokenUserDetailsCacheObj = new TokenUSerDetailsCache({ tokenId: tokenId, userIds: [oThis.userId] }),
