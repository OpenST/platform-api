...
require(rootPrefix + '/lib/cacheManagement/chain/BalanceShard');
...
class Balance extends CacheManagementChainMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.tokenHolderAddresses.length; i++) {
      lowerCaseTokenHolderAddresses = oThis.tokenHolderAddresses[i].toLowerCase();
      oThis.cacheKeys[
        oThis._cacheKeyPrefix() + 'c_c_b_' + lowerCaseTokenHolderAddresses + '_' + oThis.erc20Address.toLowerCase()
  ...
  async fetchDataFromSource(cacheMissAddresses) {
    ...
    if (!oThis.shardNumber) {
      let BalanceShardCache = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'BalanceShardCache'),
        balanceShardCacheObj = new BalanceShardCache({ erc20Address: oThis.erc20Address, chainId: oThis.chainId }),
        cacheFetchRsp = await balanceShardCacheObj.fetch();

      oThis.shardNumber = cacheFetchRsp.data['shardNumber'];
    }
    ...
    while (true) {
      let addresses = cacheMissAddresses.splice(0, BATCH_GET_LIMIT);

      promiseArray.push(
        balanceObj.getBalances({
    ...
    for (let i = 0; i < responses.length; i++) {
      Object.assign(response, responses[i].data);
    }
