...
class BalanceShard extends CacheManagementChainMultiBase {
  ...
  _setCacheKeys() {
    ...
    for (let i = 0; i < oThis.erc20Addresses.length; i++) {
      lowercaseErc20Address = oThis.erc20Address.toLowerCase();
  ...
  async fetchDataFromSource(cacheMissAddresses) {
    ...
    let tokenAddressRsp = await tokenAddressObj.getTokenIdByAddress({
      addresses: oThis.erc20Address,
      chainId: oThis.chainId
    });

    let tokenAddressData = tokenAddressRsp.data;

    // Cache empty object if no tokens
    if (!tokenAddressData) {
    ...
    for (let address in tokenAddressData) {
      ...
      tokenIdToAddressMap[tokenData.tokenId] = tokenData.address;

      promiseArray.push(
        new TokenShardNumbersCache({ tokenId: tokenData.tokenId }).fetch().then(function(resp) {
          result[tokenIdToAddressMap[tokenData.tokenId]] = resp.data[entityConst.balanceEntityKind];
        })
      );
    }
