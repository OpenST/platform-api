 ...
 *
 * @module lib/updateStakeCurrenciesTable
 */
...
class UpdateBaseCurrenciesTable {
  ...
  /**
   * Clear shared memcached.
   *
   * @return {Promise<void>}
   */
  async clearSharedCache() {
    const cacheImplementer = cache.getInstance().cacheInstance;

    await cacheImplementer.delAll();

    logger.log('Cleared shared memcached.');
  }
}
