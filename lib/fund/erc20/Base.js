...
class Base {
  ...
  async _setTokenContractAddress() {
    const oThis = this,
      stakeCurrencyBySymbolCache = new StakeCurrencyBySymbolCache({ stakeCurrencySymbols: [oThis.tokenSymbol] });

    let stakeCurrenciesCacheRsp = await stakeCurrencyBySymbolCache.fetch();

    oThis.tokenContractAddress = stakeCurrenciesCacheRsp.data[oThis.tokenSymbol].contractAddress;
  }
