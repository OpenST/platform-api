...
class RedemptionTxFinalized {
  ...
  async perform() {
    ...
    await oThis._updateRedemptionStatus();
  ...
  async _updateRedemptionStatus() {
    ...
    for (let uuid in oThis.userRedemptions) {
      ...
      if (pendingTransactionConstants.successStatus == oThis.transactionInfoMap[uuid].transactionStatus) {
        uuidsAccepted.push(uuid);
      } else {
        uuidsFailed.push(uuid);
      }
    }

    let promises = [];
    if (uuidsAccepted.length > 0) {
      promises.push(
        new UserRedemptionModel({})
          .update({
    ...
    if (uuidsFailed.length > 0) {
      promises.push(
        new UserRedemptionModel({})
          .update({
            status: userRedemptionConstants.invertedStatuses[userRedemptionConstants.redemptionFailedStatus]
          })
          .where({ uuid: uuidsFailed })
          .fire()
      );
    }
    await Promise.all(promises);
  ...
  async _fetchTokenAndProductsDetails() {
    ...
    let tokenRedemptionProductIds = [];
    for (let uuid in oThis.userRedemptions) {
      tokenRedemptionProductIds.push(oThis.userRedemptions[uuid].tokenRedemptionProductId);
    }

    const tokenRedemptionProductCacheResponse = await new TokenRedemptionProductCache({
      tokenRedemptionProductIds: tokenRedemptionProductIds
    }).fetch();

    oThis.redemptionProducts = tokenRedemptionProductCacheResponse.data;
  ...
  async _sendWebhooks() {
    ...
    for (let uuid in oThis.userRedemptions) {
      const webhookKind =
        oThis.transactionInfoMap[uuid] &&
        pendingTransactionConstants.failedStatus == oThis.transactionInfoMap[uuid].transactionStatus
          ? webhookSubscriptionsConstants.redemptionFailedTopic
          : webhookSubscriptionsConstants.redemptionAcceptedTopic;
      const payload = {
  ...
  async _sendSlackNotifications() {
    ...
    let promises = [];
    for (let uuid in oThis.userRedemptions) {
      // Don't publish on slack if its failed transaction
      if (
        oThis.transactionInfoMap[uuid] &&
        pendingTransactionConstants.failedStatus == oThis.transactionInfoMap[uuid].transactionStatus
      ) {
      ...
      const payload = {
        ...
        country: oThis.userRedemptions[uuid].countryName,
...
module.exports = RedemptionTxFinalized;
