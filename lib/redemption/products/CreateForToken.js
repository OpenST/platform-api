...
class CreateTokenRedemptionProducts {
   ...
   * @param {String} [params.instructions] - Instructions for product
   *
   * @constructor
   */
  constructor(params) {
  ...
  async _validateAndSanitize() {
    ...
    if (oThis.name && !CommonValidators.validateRedemptionProductName(oThis.name)) {
    ...
    if (oThis.description && oThis.description.length > 1000) {
      return Promise.reject(
        responseHelper.error({
  ...
  async _validateTokenId() {
    ...
    if (!CommonValidators.validateNonEmptyObject(tokenCacheResponse.data)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_r_p_cft_3',
          api_error_identifier: 'something_went_wrong',
  ...
  async _validateRedemptionProductId() {
    ...
    const redemptionProductCacheResponse = await new RedemptionProductCache({
      ids: [oThis.redemptionProductId]
    }).fetch();

    if (redemptionProductCacheResponse.isFailure()) {
      return Promise.reject(redemptionProductCacheResponse);
    }

    if (!CommonValidators.validateNonEmptyObject(redemptionProductCacheResponse.data[oThis.redemptionProductId])) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_r_p_cft_4',
          api_error_identifier: 'something_went_wrong',
          debug_options: { redemptionProductCacheResponse: redemptionProductCacheResponse }
        })
      );
    }
  }
