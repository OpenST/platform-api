...
class ValidateUserRedemptionTx {
  ...
  async _validateTokenProduct() {
    ...
    if (redeemableProducts.length <= 1 || redeemableProducts.indexOf(Number(productId)) == -1) {
  ...
  _getNumberOfBTFromFiat(fiatAmount, stakeCurrencyIsHowManyFiat) {
    ...
    const btInWei = new BigNumber(inStakeCurrency).mul(new BigNumber(oneStakeCurrencyIsHowManyBtFactor));
