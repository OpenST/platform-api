...
function signQueryParams(queryParams, _apiCredentials, queryHeaders) {
  ...
  logger.debug('==queryHeaders=====', queryHeaders);
  logger.debug('----_apiCredentials------', _apiCredentials);
  ...
  for (let i = 0; i < _apiCredentials.secrets.length; i++) {
    ...
    hmac.update(`${queryHeaders['ost-timestamp']}.${queryHeaders['ost-version']}.${queryParams}`);
...
const WebhookPostKlass = function(params) {
  ...
  oThis.requestHeaders = {
    'ost-timestamp': parseInt(Math.round(new Date().getTime() / 1000)),
    'api-signature-kind': API_SIGNATURE_KIND,
    'ost-version': params.apiVersion,
  ...
  oThis._signRequest = function(queryParams) {
    ...
    console.log('oThis.requestHeaders-------------', oThis.requestHeaders);
    oThis.requestHeaders['ost-signature'] = signQueryParams(queryParams, _apiCredentials, oThis.requestHeaders);
  };
};
