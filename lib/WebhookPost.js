...
function signQueryParams(queryParams, _apiCredentials, queryHeaders) {
  ...
  for (let i = 0; i < _apiCredentials.secrets.length; i++) {
    ...
    hmac.update(`${queryHeaders['api-request-timestamp']}.${queryHeaders.version}.${queryParams}`);
...
const WebhookPostKlass = function(params) {
  ...
  oThis.requestHeaders = {
    'api-request-timestamp': parseInt(Math.round(new Date().getTime() / 1000)),
  ...
  oThis._signRequest = function(queryParams) {
    ...
    oThis.requestHeaders['api-signature'] = signQueryParams(queryParams, _apiCredentials, oThis.requestHeaders);
  };
};
