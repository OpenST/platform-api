...
class AuxContractsSetup {
  ...
  async asyncPerform() {
    ...
    logger.step('** OST -> USD Setup Price Oracle contract: Deploy Price Oracle, Set Ops Address, Set Admin Address');
    await oThis.deployOstToUsdPriceOracle();
    await basicHelper.sleep(oThis.sleepInterval);

    logger.step('** USDC -> USD Setup Price Oracle contract: Deploy Price Oracle, Set Ops Address, Set Admin Address');
    await oThis.deployUsdcToUsdPriceOracle();
    await basicHelper.sleep(oThis.sleepInterval);
   ...
   * @returns {Promise<void>}
   */
  deployOstToUsdPriceOracle() {
    const oThis = this;

    return new DeployPriceOracle({
      auxChainId: oThis.auxChainId,
