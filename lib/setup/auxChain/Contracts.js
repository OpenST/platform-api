...
class AuxContractsSetup {
  ...
  async deployPriceOracles() {
    ...
    const stakeCurrencySymbolsResp = await new StakeCurrencySymbolsCache().fetch();

    if (stakeCurrencySymbolsResp.isFailure()) {
    ...
    let baseCurrencySymbols = stakeCurrencySymbolsResp.data;
