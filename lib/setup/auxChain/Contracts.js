...
class AuxContractsSetup {
  ...
  async initializeSTPrime() {
    ...
    return await oThis.createEconomy();
  }
  ...
  async _fetchChainAddresses() {
    ...
    let stPrimeContractResponse = await new ChainAddressModel().fetchAddresses({
      chainId: oThis.auxChainId,
      kinds: [chainAddressConstants.baseContractKind]
    });

    oThis.kindToAddressMap['contractAddress'] =
      stPrimeContractResponse.data.address[chainAddressConstants.baseContractKind];

    let simpleStakeContractResponse = await new ChainAddressModel().fetchAddresses({
      chainId: oThis.originChainId,
      auxChainId: oThis.auxChainId,
      kinds: [chainAddressConstants.simpleStakeContractKind]
    });

    oThis.kindToAddressMap['simpleStakeAddress'] =
      simpleStakeContractResponse.data.address[chainAddressConstants.simpleStakeContractKind];

    let stContractResponse = await new ChainAddressModel().fetchAddresses({
      chainId: oThis.originChainId,
      kinds: [chainAddressConstants.baseContractKind]
    });

    oThis.kindToAddressMap['originContractAddress'] =
      stContractResponse.data.address[chainAddressConstants.baseContractKind];
  }
}
