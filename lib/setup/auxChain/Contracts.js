...
class AuxContractsSetup {
  ...
  async deployPriceOracles() {
    ...
    const allStakeCurrencySymbolsResp = await new AllStakeCurrencySymbolsCache().fetch();

    if (allStakeCurrencySymbolsResp.isFailure()) {
    ...
    let baseCurrencySymbols = allStakeCurrencySymbolsResp.data;

    for (let index = 0; index < baseCurrencySymbols.length; index++) {
      let baseCurrency = baseCurrencySymbols[index];

      logger.step('* Deploying price oracle for base currency:', baseCurrency);
