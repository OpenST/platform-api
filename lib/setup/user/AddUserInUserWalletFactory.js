...
class AddUserInUserWalletFactory {
  ...
  async _performAddUser() {
    ...
    for (let index = 0; index < oThis.sessionAddresses.length; index++) {
      sessionSpendingLimits.push(oThis.sessionSpendingLimit);
      sessionExpirations.push(oThis.sessionExpiration);
    }
    ...
    let perSessionGas = new BigNumber(contractConstants.addUserInUserWalletGasPerSessionAddress).mul(
      oThis.sessionAddresses.length
    );
    let bnGas = new BigNumber(contractConstants.addUserInUserWalletGasBaseGas).plus(perSessionGas),
      gas = basicHelper.formatWeiToString(bnGas);

    console.log('---gas--', gas);
    const txOptions = {
      gasPrice: contractConstants.auxChainGasPrice,
      gas: gas, // Mandatory to pass this value.
      value: '0x0',
      from: fromAddress,
      to: toAddress,
      data: data
    ...
    const submitTransactionObj = new SubmitTransaction({
      ...
      pendingTransactionKind: pendingTransactionConstants.addUserInWalletFactoryKind,
      txOptions: txOptions,
