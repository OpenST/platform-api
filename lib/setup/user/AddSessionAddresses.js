  ...
  async _addSessionAddresses() {
    ...
    await sessionModel.batchWriteItem(insertParams).catch(function(err) {
      ...
      if (responseHelper.isCustomResult(error)) {
        return Promise.reject(
          responseHelper.error({
            internal_error_identifier: error.internalErrorCode + ':l_s_u_asa_6',
    ...
    await SessionModel.afterUpdate(oThis.ic(), { userId: oThis.userId, addresses: oThis.sessionAddresses });

    logger.debug('Cleared Sessions cache');
  }
