...
class AddSessionAddresses {
  ...
  _validateAndSanitize() {
    ...
    if (!CommonValidators.validateBigNumber(oThis.expirationHeight)) {
      paramErrors.push('invalid_expiration_height');
    }

    if (!CommonValidators.validateBigNumber(oThis.spendingLimit)) {
      paramErrors.push('invalid_spending_limit');
    }

    if (oThis.knownAddressIds) {
      if (!CommonValidators.validateIntegerArray(oThis.knownAddressIds)) {
