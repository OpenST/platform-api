...
class DeployPricerRule {
  ...
  async _asyncPerform() {
    ...
    await oThis._fetchAndSetStakeCurrencyDetails();
  ...
  async _fetchAndSetTokenDetails() {
    ...
    oThis.stakeCurrencyId = cacheResponse.data.stakeCurrencyId;
  ...
  /**
   * Fetch and set stake currency details
   *
   * @returns {Promise<never>}
   * @private
   */
  async _fetchAndSetStakeCurrencyDetails() {
    const oThis = this;
    ...
    oThis.baseCurrencyCode = stakeCurrencyCacheResponse.data[oThis.stakeCurrencyId].constants.baseCurrencyCode;
  }
  ...
  async _deployContract() {
    ...
    let txObject = await openSTJsRulesSetup._deployPricerRuleRawTx(
      oThis.baseCurrencyCode,
      conversionRateForContract,
      coreConstants.STAKE_CURRENCY_TO_BT_CONVERSION_RATE_DECIMALS,
      contractConstants.requiredPriceOracleDecimals
    );

    Object.assign(oThis.responseData, {
      baseCurrencyCode: oThis.baseCurrencyCode,
      conversionRateForContract: conversionRateForContract,
      conversionRateDecimals: coreConstants.STAKE_CURRENCY_TO_BT_CONVERSION_RATE_DECIMALS,
    ...
    let submitTxRsp = await new SubmitTransaction({
      ...
      tokenId: oThis.tokenId,
      pendingTransactionKind: pendingTransactionConstants.deployPricerRuleKind,
