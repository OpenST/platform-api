...
class InitCompanyTokenHolder {
  ...
  async _fetchExistingSessions() {
    ...
    let UserSessionList = oThis.ic().getShadowedClassFor(coreConstants.icNameSpace, 'UserSessionList'),
      userSessionListResp = await new UserSessionList({
        user_id: oThis.tokenCompanyUserId,
        token_id: oThis.tokenId
      }).perform();

    if (userSessionListResp.isFailure()) {
      return Promise.reject(userSessionListResp);
    }

    let sessionsData = userSessionListResp.data[resultType.sessions],
      buffer;
  ...
  async _fetchSessionSpendingLimit() {
    ...
    console.log('sessionSpendingLimit--------', sessionSpendingLimit);
