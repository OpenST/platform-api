...
class InitCompanyTokenHolder {
  ...
  async _fetchSessionSpendingLimit() {
    ...
    let tokenCacheResponse = await new TokenCache({ clientId: oThis.clientId }).fetch();
    ...
    let stakeCurrencyData = stakeCurrencyCacheResponse.data[tokenCacheResponse.data.stakeCurrencyId],
      companyTokenHolderSessionSpendingLimit = stakeCurrencyData['constants']['companyTokenHolderSessionSpendingLimit'];

    let BnConversionFactor = basicHelper.convertToBigNumber(tokenCacheResponse.data.conversionFactor),
    ...
    return basicHelper.formatWeiToString(BnSpendingLimit.mul(BnConversionFactor));
  }
}
