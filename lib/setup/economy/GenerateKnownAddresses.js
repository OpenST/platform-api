...
class GenerateInternalAddresses {
  ...
  constructor(params) {
    ...
    oThis.addressKinds = [
      ...
      tokenAddressConstant.tokenUserOpsWorkerKind,
      tokenAddressConstant.recoveryControllerAddressKind
    ];
  }
  ...
  async perform() {
    ...
    await oThis._createEconomySpecificSalt();
  ...
  /**
   * Create Encryption Salt for an Economy.
   *
   * @private
   */
  async _createEconomySpecificSalt() {
    const oThis = this;

    let insertedRec = await new EncryptionSaltModel().createEncryptionSalt(
      kmsConstants.userScryptSaltPurpose,
      oThis.tokenId,
      encryptionSaltConst.userEncryptionKind
    );

    if (basicHelper.isEmptyObject(insertedRec)) {
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 'l_s_e_gka_2',
          api_error_identifier: 'something_went_wrong'
        })
      );
    }
