...
class DeployUtilityBrandedToken extends Base {
  ...
  async _setAddresses() {
    ...
    oThis.deployerAddress = oThis.auxKindToAddressMap.data.address[chainAddressConstants.deployerKind];
  ...
  async _deployContract() {
    ...
    let submitTxRsp = await new SubmitTransaction({
      ...
      provider: oThis._configStrategyObject.chainWsProvider(oThis.deployChainId, 'readWrite'),
      txOptions: txOptions,
