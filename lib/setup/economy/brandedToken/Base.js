...
class DeployBrandedTokenBase {
  ...
  async _asyncPerform() {
    ...
    const deploymentResponse = await oThis._deployContract();
  ...
  async _fetchAddresses(associatedAuxChainId) {
    ...
    const chainAddressCacheObj = new ChainAddressCache({ associatedAuxChainId: associatedAuxChainId }),
  ...
  async _fetchAndSetTokenDetails() {
    ...
    const tokenDetails = cacheResponse.data;
  ...
  async _fetchAndSetGasPrice(chainKind) {
    ...
    switch (chainKind) {
      ...
      default:
        throw new Error(`Unsupported chainKind: ${chainKind}`);
    }
  }
