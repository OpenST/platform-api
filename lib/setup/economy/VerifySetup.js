...
class EconomySetupVerifier {
  ...
  async _validateAuxTokenOrganization() {
    ...
    if (!owner || owner.toLowerCase() !== oThis.tokenAddresses[tokenAddressConstants.ownerAddressKind].toLowerCase()) {
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 't_v_es_9',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
            contractValue: owner,
    ...
    if (!isWorker) {
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 't_v_es_11',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
  ...
  async _validateOriginTokenOrganization() {
    ...
    if (!owner || owner.toLowerCase() !== oThis.tokenAddresses[tokenAddressConstants.ownerAddressKind].toLowerCase()) {
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 't_v_es_12',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
            contractValue: owner,
            dbValue: oThis.tokenAddresses[tokenAddressConstants.ownerAddressKind]
    ...
    if (
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 't_v_es_13',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
            contractValue: admin,
    ...
    if (!isWorker) {
      ...
      return Promise.reject(
        responseHelper.error({
          internal_error_identifier: 't_v_es_14',
          api_error_identifier: 'something_went_wrong',
          debug_options: {
            contractWorkerValue: isWorker
          }
