...
class EconomySetupVerifier {
  ...
  async _fetchTokenDetails() {
    ...
    oThis.tokenDetails = response.data;

    let stakeCurrencyId = oThis.tokenDetails.stakeCurrencyId;
    ...
    logger.debug('oThis.stakeCurrencyDetails-------', oThis.stakeCurrencyDetails);
  }
  ...
  async _validatePricerRule() {
    ...
    if (
      !baseCurrencyCode ||
      basicHelper.convertHexToString(baseCurrencyCode.slice(2)) !== oThis.stakeCurrencyDetails.symbol
    ) {
      ...
      return Promise.reject(
        responseHelper.error({
          ...
          debug_options: {
            ...
            stakeCurrencySymbol: oThis.stakeCurrencyDetails.symbol
          }
        })
      );
    }
