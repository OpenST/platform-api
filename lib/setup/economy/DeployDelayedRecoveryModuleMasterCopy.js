/**
 * Deploy Delayed Recovery Module Master Copy
 *
 * @module lib/setup/economy/DeployDelayedRecoveryModuleMasterCopy
 */
...
/**
 * Class for Delayed Recovery Module Master Copy deployment
 *
 * @class
 */
class DeployDelayedRecoveryModuleMasterCopy {
  /**
   * Constructor for Delayed Recovery Module Master Copy deployment
   *
   * @param {Object} params
    ...
    return oThis._asyncPerform().catch(function(error) {
      if (responseHelper.isCustomResult(error)) {
        ...
        return responseHelper.error({
          internal_error_identifier: 'l_s_e_ddrmmc_1',
  ...
  async _deployContract() {
    ...
    let txOptions = {
      ...
      gas: contractConstants.deployDelayedRecoveryModuleMasterCopyGas,
      value: contractConstants.zeroValue
    };

    let txObject = await openStJsUserSetupHelper._deployDelayedRecoveryModuleMasterCopyRawTx();
    ...
    let submitTxRsp = await new SubmitTransaction({
      ...
      pendingTransactionKind: pendingTransactionConstants.deployDelayedRecoveryModuleMasterCopyKind,
