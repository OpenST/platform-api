...
class AddAuxConfig extends Base {
  ...
  async _getOrCreateConfigGroup() {
    ...
    let getResp = await new ConfigGroupsModel().fetchRecord(oThis.chainId, oThis.groupId).catch(function() {
      return null;
    });

    if (!getResp) {
      let configGroup = new ConfigGroupsModel();
      return await new ConfigGroupsModel()
