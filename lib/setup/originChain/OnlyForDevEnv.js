...
class OnlyForDevEnv {
  ...
  async _asyncPerform() {
    ...
    return Promise.resolve('Dev Env related funding successfully performed.');
  }
  ...
  async _fundGranterAddressWithEth() {
    ...
    if(oThis.ethSenderPk){
      await transferAmountOnChain._fundAddressWithEthUsingPk(oThis.granterAddress, oThis.ethSenderPk, oThis.chainId, provider, amountInWei);
    } else {
