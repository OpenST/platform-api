/**
 * Module to deploy USDC stable coin.
 *
...
/**
 * Class to deploy USDC stable coin.
 *
 * @class DeployUsdcStableCoin
 */
class DeployUsdcStableCoin {
  /**
   * Constructor to deploy USDC stable coin.
   *
  ...
  perform() {
    ...
    return oThis.asyncPerform().catch(function(error) {
      ...
      logger.error('lib/setup/originChain/DeployUsdcStableCoin.js::perform::catch');
      logger.error(error);
  ...
  async asyncPerform() {
    ...
    await oThis.fundAddressWithEth('10000000000');

    await oThis.fetchSignerKey();

    await oThis.getDeploymentConfig();
    ...
    await oThis.initializeContract();
    ...
    return responseHelper.successWithData({
      ...
      stableCoinDeployerAddress: oThis.signerAddress,
      stableCoinDeployerPrivateKey: oThis.signerKey
    });
  }
  ...
  async fetchSignerAddress() {
    ...
    oThis.signerAddress = chainAddressesRsp.data[chainAddressConstants.originStableCoinDeployerKind].address;
  }
   ...
   */
  async fundAddressWithEth(amount) {
    const oThis = this;

    const provider = await oThis._getProviderFromConfig(),
      amountInWei = amount.toString(10);

    await new TransferEthUsingPK({
      toAddress: oThis.signerAddress,
  ...
  /**
   * Get the deployment config for USDC.
   *
   * @sets oThis.deploymentConfig
   */
  getDeploymentConfig() {
    const oThis = this;

    oThis.deploymentConfig = {
      name: 'USDC',
      symbol: 'USDC',
      currency: 'USD',
      decimals: 6,
      masterMinter: oThis.signerAddress,
      pauser: oThis.signerAddress,
      blacklister: oThis.signerAddress,
      owner: oThis.signerAddress
    };
  }
  ...
  async _getProviderFromConfig() {
    ...
    return readWriteConfig.wsProvider ? readWriteConfig.wsProvider : readWriteConfig.rpcProvider;
  }
  ...
  async deployContract() {
    ...
    const deployParams = {
      ...
      gas: contractConstants.deployUsdcGas,
...
module.exports = DeployUsdcStableCoin;
