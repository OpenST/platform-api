...
class FundGranterAddress {
  ...
  async _asyncPerform() {
    ...
    logger.step(`* USDC Token Owner funding granter address with ${oThis.stAmount} USDC.`);
    //await oThis._fundGranterAddressWithUSDC(); // From ST owner.
  ...
  async _fundGranterAddressWithUSDC() {
    ...
    const providers = await oThis._getProvidersFromConfig(),
      provider = providers[0], // Select one provider from provider endpoints array
      amountInWei = basicHelper.convertToWei(String(oThis.usdcAmount)).toString(10); // Transfer amount
