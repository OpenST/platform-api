...
class FundGranterAddress {
  ...
  async _fundGranterAddressWithEth() {
    ...
    const providers = await oThis._getProvidersFromConfig(),
      ...
      amountInWei = basicHelper
        .convertToLowerUnit(String(oThis.ethAmount), coreConstants.ETH_CONVERSION_DECIMALS)
        .toString(10); // Transfer amount.
  ...
  async _fundGranterAddressWithOst() {
    ...
    const providers = await oThis._getProvidersFromConfig(),
      provider = providers[0], // Select one provider from provider endpoints array
      amountInWei = basicHelper
        .convertToLowerUnit(String(oThis.stAmount), coreConstants.ETH_CONVERSION_DECIMALS)
        .toString(10); // Transfer amount
  ...
  async _fundGranterAddressWithUsdc() {
    ...
    const providers = await oThis._getProvidersFromConfig(),
      provider = providers[0], // Select one provider from provider endpoints array.
      amountInWei = basicHelper
        .convertToLowerUnit(String(oThis.usdcAmount), coreConstants.USDC_CONVERSION_DECIMALS)
        .toString(10); // Transfer amount.
