...
class SaveStakeCurrencyDetails {
  ...
  async _asyncPerform() {
    ...
    logger.step('** Update base currency contract details into stake currencies table.');
    await oThis._updateInStakeCurrencies();
   ...
   * @private
   */
  async _updateInStakeCurrencies() {
    const oThis = this;

    await new UpdateStakeCurrenciesTable(oThis.contractSymbol, oThis.contractAddress).perform();
  }
  ...
  async _insertInBaseCurrencies() {
    ...
    await new UpdateBaseCurrenciesTable(oThis.contractSymbol, oThis.contractAddress).perform();
  }
}
