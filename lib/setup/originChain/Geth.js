...
class OriginGethSetup {
  ...
  async _asyncPerform() {
    ...
    logger.step('** Generating Master Internal Funder');
    await oThis._generateMasterInternalFunderAddress();
    ...
    await oThis._fundAddressWithEth(oThis.originDeployerAddress, 0.0072);

    // The below transfers are commented right now as they don't require ETH now.
    // Uncomment when they're being used.

    // logger.log('* Funding st org contract owner address with ETH.');
    // await oThis._fundAddressWithEth(oThis.stOrgContractOwnerAddress, 0.00300);
    //
    // logger.log('* Funding origin anchor org contract owner address with ETH.');
    // await oThis._fundAddressWithEth(oThis.originAnchorOrgContractOwnerAddress, 0.00150);
    //
    ...
    //
    // logger.log('* Funding origin default BT org contract admin address with ETH.');
    // await oThis._fundAddressWithEth(oThis.originDefaultBTOrgContractAdminAddress, 0.00240);
    //
    // logger.log('* Funding origin default BT org contract worker address with ETH.');
  ...
  /**
   * Generate Master internal funder
   *
   * @returns {Promise<Promise<never> | Promise<any>>}
   *
   * @private
   */
  async _generateMasterInternalFunderAddress() {
    const oThis = this;

    let generateChainKnownAddresses = new GenerateChainKnownAddresses({
      addressKinds: [chainAddressConstants.masterInternalFunderKind],
    ...
    if (!generateOriginAddrRsp.isSuccess()) {
      logger.error('Generating master internal funder kind address failed');
   ...
   * @param amount {Number/BigNumber} - amount in eth which is to be funded
   *
   * @returns {Promise<void>}
   *
   * @private
