...
class OriginGethSetup {
  ...
  async _asyncPerform() {
    ...
    await oThis._fundAddressWithEth(oThis.originDeployerAddress, 2);

    logger.log('* Funding st org contract owner address with ETH.');
    await oThis._fundAddressWithEth(oThis.stOrgContractOwnerAddress, 2);

    logger.log('* Funding origin anchor org contract owner address with ETH.');
    await oThis._fundAddressWithEth(oThis.originAnchorOrgContractOwnerAddress, 2);
    ...
    await oThis._fundAddressWithEth(oThis.originDefaultBTOrgContractAdminAddress, 2);

    logger.log('* Funding origin default BT org contract worker address with ETH.');
    await oThis._fundAddressWithEth(oThis.originDefaultBTOrgContractWorkerAddress, 2);
  ...
  async _fundAddressWithEth(address, amount) {
    ...
    let providers = await oThis._getProvidersFromConfig(),
      ...
      amountInWei = basicHelper.convertToWei(amount).toString(10);
