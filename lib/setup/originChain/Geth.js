...
class OriginGethSetup {
  ...
  async _asyncPerform() {
    ...
    await oThis._fundAddressWithEth(oThis.ownerAddr, 0.5);

    logger.log('* Funding origin admin address with ETH.');
    await oThis._fundAddressWithEth(oThis.adminAddr, 0.5);

    logger.log('* Funding origin token admin address with ETH.');
    await oThis._fundAddressWithEth(oThis.tokenAdminAddr, 0.5);

    logger.log('* Funding origin token worker address with ETH.');
    await oThis._fundAddressWithEth(oThis.tokenWorkerAddr, 0.5);
  ...
  async _generateOriginAddr() {
    ...
    oThis.tokenWorkerAddr = addresses[chainAddressConstants.tokenWorkerKind];
