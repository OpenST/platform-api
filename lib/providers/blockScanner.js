...
class BlockScannerProvider {
  ...
  async _getBlockScannerConfigStrategy(chainIds) {
    ...
    for (let chainId in chainCompleteConfigs) {
      ...
      //TODO: now these below vars can come from configStrategyObj
      ...
      let chainConfigArrayElement = {
        ...
        storage: chainFullConfig[configStrategyConstants.dynamodb], // Conditional
        nodes: nodes
      };
  ...
  /**
   * If chain is origin then chain cache config will be globalMemcached
  ...
  _getCacheConfigForChain(chainFullConfig, isOriginChain) {
    if (isOriginChain) {
      return chainFullConfig.globalMemcached;
    } else {
