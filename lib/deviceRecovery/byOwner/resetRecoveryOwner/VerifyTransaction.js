  ...
  async perform() {
    ...
    await oThis._updateRecoveryOwnerStatuses(transactionVerified).catch(async function(err) {
      ...
      const errorObject = responseHelper.error({
        internal_error_identifier: 'recovery_owner_status_not_updated:l_dr_bo_rco_vt_4',
    ...
    if (transactionVerified) {
      ...
      if (updateRecoveryOwnerAddress.isFailure()) {
        ...
        const errorObject = responseHelper.error({
          internal_error_identifier: 'user_recovery_owner_not_updated:l_dr_bo_rco_vt_3',
