...
class VerifyTransaction extends DeviceRecoveryBase {
  ...
  async perform() {
    ...
    await oThis._updateRecoveryOwnerStatuses(transactionVerified).catch(async (err) => {
      logger.error('Could not update recovery owners statuses:', err);
      await basicHelper.notify('l_dr_bo_rco_vt_4', 'Could not update recovery owners statuses.', err);
    ...
    if (transactionVerified) {
      ...
      if (updateRecoveryOwnerAddress.isFailure()) {
        ...
        await basicHelper.notify(
          ...
          {},
          { address: updateRecoveryOwnerAddress }
        );
      }
    }
