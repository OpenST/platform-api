...
class Base {
  ...
  async _performTransactionFailedSteps() {
    ...
    await emailNotifier.perform(
      'l_dr_b_1',
      `Transaction was not performed for recovery operation Id: ${oThis.recoveryOperationId}`,
      {},
      {}
    );
  }
  ...
  get _pendingTransactionKind() {
    throw 'if a sub class calls _submitTransaction, it needs to implement this and return pendingTransactionKind';
  }
}
