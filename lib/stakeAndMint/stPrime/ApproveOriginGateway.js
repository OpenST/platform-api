...
class ApproveOriginGatewayInBase extends StakeAndMintBase {
  ...
  async _fetchRequiredAddresses() {
    ...
    oThis.gatewayContract = response.data[chainAddressConstants.originGatewayContractKind];
    ...
    response = await new ChainAddressModel().fetchAddress(params);

    oThis.baseContract = response.data[chainAddressConstants.baseContractKind];
  }
