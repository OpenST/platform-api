...
/*
 *
 *  @module lib/stakeAndMint/MerkleProofGenerator.js
 */
...
class MerkleProofGenerator {
   ...
   *
   * @param params {Object}
   * @param params.web3Instance     {Object} Web3 Instance
   * @param params.contractAddress  {String} Contract Address
   * @param params.blockNumber      {Number} Block Number to generate proof for.
   * @param params.messageHash      {Number} Stake Intent declared event message hash to generate storage proof.
   *
   */
  constructor(params) {
    const oThis = this;
    ...
    if (params.messageHash) {
      oThis.proofInputKeys = { [oThis.contractAddress]: [params.messageHash] };
    } else {
      oThis.proofInputKeys = { [oThis.contractAddress]: [] };
    }
  }
    ...
    return oThis._asyncPerform().catch(function(error) {
      if (responseHelper.isCustomResult(error)) {
        ...
        return responseHelper.error({
          internal_error_identifier: 'l_smm_mpg_1',
    ...
    if (!oThis.contractAddress) {
      return Promise.resolve(
        responseHelper.error({
          internal_error_identifier: 'l_smm_mpg_2',
  ...
  _storagePath(storageIndex, mappings) {
    ...
    path = oThis.web3Instance.utils.sha3(path, { encoding: 'hex' });

    return path;
  }

  /**
   * Generate RLP account
   *
