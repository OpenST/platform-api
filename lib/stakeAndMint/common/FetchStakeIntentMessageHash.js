...
class FetchStakeIntentMessageHash {
  ...
  constructor(params) {
    ...
    oThis.stakeIntentDeclaredEventDetails = null;
  }
  ...
  async _getTxReceipt() {
    ...
    oThis.txReceipt = await oThis.web3Instance.eth.getTransactionReceipt(txHash);
  ...
  _addABI() {
    ...
    let abi = new MosaicTbd.AbiBinProvider().getABI('EIP20Gateway');
