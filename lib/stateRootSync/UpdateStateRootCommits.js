...
class UpdateStateRootCommit {
  ...
  async _asyncPerform() {
    ...
    if (response.affectedRows == 1) {
      ...
      return responseHelper.successWithData({ taskStatus: workflowStepConstants.taskPending });
    }
  }
  ...
  async _getTransactionStatus() {
    ...
    oThis.status = checkStatusResponse.data
      ? stateRootCommitHistoryConstants.commited
      : stateRootCommitHistoryConstants.failed;

    // TODO: If transaction status is failed then check for commited state root and decide.
  }
