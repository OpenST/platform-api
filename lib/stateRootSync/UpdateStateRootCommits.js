...
class UpdateStateRootCommit extends StateRootSyncBase {
  ...
  async _asyncPerform() {
    ...
    if (txStatus.isFailure() || !oThis.blockNumber) {
      ...
      if (oThis.blockNumber && oThis.blockNumber > blockNumberFromContract) {
        ...
        successDebugParams = {
          ...
          latestStateRootBlockHeightFromContract: blockNumberFromContract,
          Msg:
            'State root commit passed because the block number passed is smaller than latest state root block height.'
        };
