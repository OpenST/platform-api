...
class StateRootCommit {
  ...
  async _asyncPerform() {
    ...
    if (response.affectedRows == 1) {
      return responseHelper.successWithData({ taskDone: 1, taskResponseData: { chainId: oThis.destinationChainId } });
    } else {
      return responseHelper.successWithData({ taskDone: 0, taskResponseData: { chainId: oThis.destinationChainId } });
    }
  }
  ...
  async _getTransactionStatus() {
    ...
    let checkTxStatus = new CheckTxStatus({
      transactionReceipt: response.data[oThis.transactionHash]
    });
