  ...
  async _commitTransaction() {
    ...
    if (!basicHelper.isDevelopment()) {
      if (oThis.destinationChainId == 3 || oThis.destinationChainId == 1) {
        // Ropsten.
        if (parseInt(oThis.blockNumberToBeCommitted) > 10000000) {
          const errMsg = 'Aux chain block number ' + oThis.blockNumberToBeCommitted + ' is greater than 10000000';
          logger.error(errMsg);
        ...
        // AUX Chain.
        if (parseInt(oThis.blockNumberToBeCommitted) < 6400000) {
          const errMsg = 'Origin chain block number ' + oThis.blockNumberToBeCommitted + ' is less than 6400000';
          logger.error(errMsg);
